...
subroutine usage()
 ...
 implicit none
  write(stdout, *) 'aquachem.x - standalone driver for aquachem'
  write(stdout, *) 'usage: aquachem.x namelist_filename'
...
! initialize history file
  if(salton)then
    prefix='aquachem.salt'
  else
    prefix='aquachem.nosalt'
  endif
  if(len(trim(case_id))==0)then
    write(gname,'(A)')trim(prefix)//'.'//trim(model_name)
  else
    write(gname,'(A)')trim(prefix)//'.'//trim(case_id)//'.'//trim(model_name)
  endif
