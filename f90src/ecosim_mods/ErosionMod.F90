...
!     WRITE(*,1117)'DETR',I,J,M,NX,NY,RDTSED(NY,NX),SED(NY,NX),DETR
...
!
      ...
      IF(BKDS(NU(NY,NX),NY,NX).GT.ZERO &
      .AND.SEDX.GT.ZEROS(NY,NX) &
      .AND.XVOLTM(M,NY,NX).LE.VOLWG(NY,NX) &
      .AND.FERSNM(NY,NX).GT.ZERO)THEN
      CSEDD=AMAX1(0.0,SEDX/XVOLWM(M,NY,NX))
      DEPI=AMAX1(-SEDX &
      ,VLS(NY,NX)*(0.0-CSEDD)*AREA(3,NU(NY,NX),NY,NX) &
      *FERSNM(NY,NX)*FMPR(NU(NY,NX),NY,NX)*XNPH)
      RDTSED(NY,NX)=RDTSED(NY,NX)+DEPI
!     WRITE(*,1117)'DEPI',I,J,M,NX,NY,RDTSED(NY,NX),SEDX,DEPI
!    2,CSEDD,FVOLIM(NY,NX),FVOLWM(NY,NX),FERSNM(NY,NX)
!    3,VLS(NY,NX),XVOLWM(M,NY,NX)
      ENDIF
!
...
!
      IF(BKDS(NU(NY,NX),NY,NX).GT.ZERO &
      .AND.XVOLTM(M,NY,NX).GT.VOLWG(NY,NX) &
      .AND.FERSNM(NY,NX).GT.ZERO)THEN
...
!     WRITE(*,1112)'DETI',I,J,M,NX,NY,RDTSED(NY,NX),SED(NY,NX),DETI
...
!     IF(N2.EQ.5)THEN
!     WRITE(*,1113)'INTB',I,J,M,N1,N2,N4B,N5B,N
!    2,RERSED0(N2,N1),RERSED(N,1,N5B,N4B),XSEDER(N,1,N5B,N4B)
!    3,SED(N2,N1),XVOLWM(M,N2,N1)
!     ENDIF
      ELSE
