module RedistMod
  ...
  integer :: IFLGLS,ICHKLX,ICHKL,LG,LX,LS,LS2
...
!     TOTAL FLUXES FROM SEDIMENT TRANSPORT
      call TotalFluxFromSedmentTransp(N,N1,N2 &
      ,N4,N5,N4B,N5B,NY,NX)
      ENDIF
!
...
!     watch out 'L' in the following call
      call AdjustSoilProfile(L,NY,NX)
...
!    I,HTHAWR(NY,NX),HFLXD,HFLXO,4.19*TKA(NY,NX)*PRECA(NY,NX)
!    I,2.095*TKA(NY,NX)*PRECW(NY,NX),HEATH(NY,NX),THFLXC(NY,NX)
!    I,(XTHAWW(L,NY,NX),L=1,JS)
!    I,(THTHAW(L,NY,NX),L=NU(NY,NX),NL(NY,NX))
!    I,(TUPHT(L,NY,NX),L=NU(NY,NX),NL(NY,NX))
!    O,4.19*TKA(NY,NX)*PRECU(NY,NX),THQS(NY,NX)
...
!
      ...
      .AND.VHCPW(1,NY,NX).LE.VHCPWX(NY,NX) &
...
!
  VHCPZ=VHCP(0,NY,NX)
  VHCPY=4.19_r8*VOLW(0,NY,NX)+1.9274_r8*VOLI(0,NY,NX) &
      +2.496E-06_r8*ORGC(0,NY,NX)
  VHCPO=VHCPY-VHCPZ
  ...
  VHCP(0,NY,NX)=2.496E-06_r8*ORGC(0,NY,NX)+4.19_r8*VOLW(0,NY,NX) &
  ...
  IF(VHCP(0,NY,NX).GT.VHCPRX(NY,NX))THEN
    ...
    HEATIN=HEATIN+HFLXO
!    if(TKS(0,NY,NX)>350._r8 .or. curday>=365)then
!      write(*,*)'\n'
...
!
  HEATIN=HEATIN+4.19_r8*TKA(NY,NX)*PRECA(NY,NX) &
      +2.095_r8*TKA(NY,NX)*PRECW(NY,NX)
...
!
  SIN=((FLQGQ(NY,NX)+FLQRQ(NY,NX)) &
      *(2.0*CN4R(NY,NX)+CN3R(NY,NX)+CNOR(NY,NX)) &
      +(FLQGI(NY,NX)+FLQRI(NY,NX)) &
      *(2.0*CN4Q(I,NY,NX)+CN3Q(I,NY,NX)+CNOQ(I,NY,NX)))
  SGN=(2.0*(FLQGQ(NY,NX)+FLQRQ(NY,NX))*(CNNR(NY,NX)+CN2R(NY,NX)) &
      +2.0*(FLQGI(NY,NX)+FLQRI(NY,NX))*(CNNQ(NY,NX)+CN2Q(NY,NX)) &
      +2.0*(XNGDFS(NY,NX)+XN2DFS(NY,NX))+XN3DFS(NY,NX) &
      +XNBDFS(NY,NX)+2.0*(XNGFLG(3,NU(NY,NX),NY,NX) &
      +XN2FLG(3,NU(NY,NX),NY,NX))+XN3FLG(3,NU(NY,NX),NY,NX) &
      +2.0*TN2OZ(NY,NX)+TNH3Z(NY,NX) &
      +2.0*(XN2DFG(0,NY,NX)+XNGDFG(0,NY,NX))+XN3DFG(0,NY,NX) &
      +2.0*(XNGDFR(NY,NX)+XN2DFR(NY,NX))+XN3DFR(NY,NX))/14.0
  SIP=((FLQGQ(NY,NX)+FLQRQ(NY,NX)) &
      *(3.0*CPOR(NY,NX)+2.0*CH1PR(NY,NX)) &
      +(FLQGI(NY,NX)+FLQRI(NY,NX)) &
      *(3.0*CPOQ(I,NY,NX)+2.0*CH1PQ(I,NY,NX)))
...
!
  IF(ISALTG.NE.0)THEN
    SIR=PRECQ(NY,NX)*(CALR(NY,NX)+CFER(NY,NX)+CHYR(NY,NX)+CCAR(NY,NX) &
      ...
      +2.0*(CHCR(NY,NX)+CAL1R(NY,NX)+CALSR(NY,NX)+CFE1R(NY,NX) &
      ...
      +3.0*(CAL2R(NY,NX)+CFE2R(NY,NX)+CCAHR(NY,NX)+CMGHR(NY,NX) &
      +CF1PR(NY,NX)+CC1PR(NY,NX)+CM1PR(NY,NX)) &
      +4.0*(CAL3R(NY,NX)+CFE3R(NY,NX)+CH3PR(NY,NX)+CF2PR(NY,NX) &
      +CC2PR(NY,NX)) &
      +5.0*(CAL4R(NY,NX)+CFE4R(NY,NX)))
    SII=PRECI(NY,NX)*(CALQ(I,NY,NX)+CFEQ(I,NY,NX)+CHYQ(I,NY,NX) &
      ...
      +CH0PQ(I,NY,NX) &
      +2.0*(CHCQ(I,NY,NX)+CAL1Q(I,NY,NX)+CALSQ(I,NY,NX) &
      ...
      +3.0*(CAL2Q(I,NY,NX)+CFE2Q(I,NY,NX)+CCAHQ(I,NY,NX) &
      +CMGHQ(I,NY,NX)+CF1PQ(I,NY,NX)+CC1PQ(I,NY,NX)+CM1PQ(I,NY,NX)) &
      +4.0*(CAL3Q(I,NY,NX)+CFE3Q(I,NY,NX) &
      +CH3PQ(I,NY,NX)+CF2PQ(I,NY,NX)+CC2PQ(I,NY,NX)) &
      +5.0*(CAL4Q(I,NY,NX)+CFE4Q(I,NY,NX)))
...
!
    SBU=-PRECU(NY,NX)*(CALQ(I,NY,NX)+CFEQ(I,NY,NX)+CHYQ(I,NY,NX) &
      ...
      +CCASQ(I,NY,NX)+CMGOQ(I,NY,NX)+CMGCQ(I,NY,NX)+CMGSQ(I,NY,NX) &
      +CNACQ(I,NY,NX)+CNASQ(I,NY,NX)+CKASQ(I,NY,NX)+CC0PQ(I,NY,NX)) &
      +3.0*(CAL2Q(I,NY,NX)+CFE2Q(I,NY,NX)+CCAHQ(I,NY,NX)+CMGHQ(I,NY,NX) &
...
!     VOLT(L,NY,NX)=VOLA(L,NY,NX)
      ...
      +4.19*(VOLW(L,NY,NX)+VOLWH(L,NY,NX)) &
      +1.9274*(VOLI(L,NY,NX)+VOLIH(L,NY,NX))
...
!------------------------------------------------------------------------------------------
      ...
      subroutine AdjustSoilProfile(L,NY,NX)
...
!
      ...
      .AND.XVOLWP.GT.VOLWD(NY,NX)+VHCPNX(NY,NX)/4.19)THEN
...
!     ENDIF
      ...
      +4.19*(VOLW(L1,NY,NX)+VOLWH(L1,NY,NX)) &
      +1.9274*(VOLI(L1,NY,NX)+VOLIH(L1,NY,NX))
...
!     ENDIF
      ...
      +4.19*(VOLW(L0,NY,NX)+VOLWH(L0,NY,NX)) &
      +1.9274*(VOLI(L0,NY,NX)+VOLIH(L0,NY,NX))
      ELSE
      VHCP(L0,NY,NX)=VHCM(L0,NY,NX) &
      +4.19*VOLW(L0,NY,NX)+1.9274*VOLI(L0,NY,NX)
      ENDIF
...
!     ENDIF
      ...
      +FXVHCM+4.19*FXVOLW+1.9274*FXVOLI
...
245   CONTINUE
      ENDIF
      end subroutine AdjustSoilProfile
!------------------------------------------------------------------------------------------
...
!     TVOLA=TVOLA+TI*VOLA(L,NY,NX)
      ...
      +1.9274*(VOLI(L,NY,NX)+VOLIH(L,NY,NX)))*TKS(L,NY,NX)
