...
!
      ...
      ,VMXC=0.015,FSNR=2.884E-03,FLG4X=168.0 &
      ,FLGZX=240.0,XFRX=2.5E-02,XFRY=2.5E-03 &
      ,FSNK=0.05,FXFS=1.0,FMYC=0.1
...
!
      ...
      ,RTDPX=0.00,RTLGAX=1.0E-03,EMODR=5.0
!
...
!
      ...
      ,CURV4=4.0*CURV,ELEC3=4.5,ELEC4=3.0,CO2KI=1.0E+03,FCO2B=0.02 &
      ,FHCOB=1.0-FCO2B
...
!
      ...
      ,ZPGRD=1.0-ZPGRM,GY=1.0,GZ=1.0-GY
      real(r8), PARAMETER :: FSTK=0.05,ZSTX=1.0E-03,DSTK=0.225 &
      ,VSTK=1.0E-06/DSTK,FRTX=1.0/(1.0-(1.0-FSTK)**2)
!
...
!
      ...
      ,SPNDL=5.0E-04,CCNGR=2.5E-01,CCNGB=6.0E-04 &
      ,WTNDI=1.0E-03,CZKM=2.5E-03,CPKM=2.5E-04 &
      ,RCCZR=0.056,RCCYR=0.167,RCCXR=0.833,RCCQR=0.833 &
      ,RCCZN=0.167,RCCYN=0.833,RCCXN=0.833,RCCQN=0.833
...
!     IF(FHVSTK(K).EQ.0.0.AND.K.GT.0)THEN
!     IF(IBTYP(NZ,NY,NX).EQ.0.OR.IGTYP(NZ,NY,NX).LE.1)THEN
...
!
      ...
      .AND.NB.EQ.NB1(NZ,NY,NX).AND.SNCF.EQ.0)THEN
      NBY=0
...
560   CONTINUE
      ...
      ENDIF
      HTLFB=HTBR &
      +AMAX1(0.0,HTNODE(K1,NB,NZ,NY,NX))
!
...
!
      ...
      +STMX(NZ,NY,NX)*AMAX1(0.0,GROSTK)
...
!
      ...
      .OR.(ISTYP(NZ,NY,NX).EQ.0.AND.IWTYP(NZ,NY,NX).NE.0))THEN
      IF((IFLGE(NB,NZ,NY,NX).EQ.0 &
      .AND.VRNS(NB,NZ,NY,NX).GE.VRNL(NB,NZ,NY,NX)) &
      .OR.(IFLGF(NB,NZ,NY,NX).EQ.0 &
      .AND.VRNF(NB,NZ,NY,NX).GE.VRNX(NB,NZ,NY,NX)))THEN
!
...
!
      ...
      DO 6330 M=1,4
      CSNC(M,1,0,NZ,NY,NX)=CSNC(M,1,0,NZ,NY,NX) &
      +FSNR*CFOPC(2,M,NZ,NY,NX) &
      *(WTHSKB(NB,NZ,NY,NX)+WTEARB(NB,NZ,NY,NX))
      ZSNC(M,1,0,NZ,NY,NX)=ZSNC(M,1,0,NZ,NY,NX) &
      +FSNR*CFOPN(2,M,NZ,NY,NX) &
      *(WTHSBN(NB,NZ,NY,NX)+WTEABN(NB,NZ,NY,NX))
      PSNC(M,1,0,NZ,NY,NX)=PSNC(M,1,0,NZ,NY,NX) &
      +FSNR*CFOPP(2,M,NZ,NY,NX) &
      *(WTHSBP(NB,NZ,NY,NX)+WTEABP(NB,NZ,NY,NX))
      IF(ISTYP(NZ,NY,NX).EQ.0.AND.IWTYP(NZ,NY,NX).NE.0)THEN
      WTRVC(NZ,NY,NX)=WTRVC(NZ,NY,NX) &
...
!
      IF((IBTYP(NZ,NY,NX).EQ.0.OR.IGTYP(NZ,NY,NX).LE.1) &
      .AND.ISTYP(NZ,NY,NX).NE.0)THEN
      DO 6335 M=1,4
      CSNC(M,1,0,NZ,NY,NX)=CSNC(M,1,0,NZ,NY,NX) &
      +FSNR*CFOPC(3,M,NZ,NY,NX)*WTSTKB(NB,NZ,NY,NX)
      ZSNC(M,1,0,NZ,NY,NX)=ZSNC(M,1,0,NZ,NY,NX) &
      +FSNR*CFOPN(3,M,NZ,NY,NX)*WTSTBN(NB,NZ,NY,NX)
      PSNC(M,1,0,NZ,NY,NX)=PSNC(M,1,0,NZ,NY,NX) &
      +FSNR*CFOPP(3,M,NZ,NY,NX)*WTSTBP(NB,NZ,NY,NX)
6335  CONTINUE
...
!
      ...
      .AND.(IWTYP(NZ,NY,NX).EQ.1.OR.IWTYP(NZ,NY,NX).EQ.3))THEN
...
!     CH2OH=storage C oxidation rate during leafout
!     FRSV=rate constant for remobiln of storage C,N,P during leafout C     FXSH=shoot partitioning of storage C during leafout
!
      IF(WTRVC(NZ,NY,NX).GT.ZEROP(NZ,NY,NX))THEN
      IF(ISTYP(NZ,NY,NX).NE.0)THEN
      CPOOLT=AMAX1(0.0,WTRVC(NZ,NY,NX)+CPOOL(NB,NZ,NY,NX))
      ZPOOLD=(WTRVN(NZ,NY,NX)*CPOOL(NB,NZ,NY,NX) &
      -ZPOOL(NB,NZ,NY,NX)*WTRVC(NZ,NY,NX))/CPOOLT
      PPOOLD=(WTRVP(NZ,NY,NX)*CPOOL(NB,NZ,NY,NX) &
      -PPOOL(NB,NZ,NY,NX)*WTRVC(NZ,NY,NX))/CPOOLT
      ...
      *CH2OH*WTRVN(NZ,NY,NX)/WTRVC(NZ,NY,NX))
      UPPO4B=AMAX1(0.0,FXSH(ISTYP(NZ,NY,NX)) &
      *CH2OH*WTRVP(NZ,NY,NX)/WTRVC(NZ,NY,NX))
      ENDIF
      ELSE
...
3     CONTINUE
      ...
      -ZPOOLM*WTRVC(NZ,NY,NX))/CPOOLT
      PPOOLD=(WTRVP(NZ,NY,NX)*CPOOLM &
      -PPOOLM*WTRVC(NZ,NY,NX))/CPOOLT
...
!
      ...
      .AND.CPOOLM.GT.ZEROP(NZ,NY,NX))THEN
...
!
      ...
      *(0.5*(CPOOL(NB1(NZ,NY,NX),NZ,NY,NX)+CPOOL(NB,NZ,NY,NX)) &
      -CPOOL(NB,NZ,NY,NX)))
      XFRN=AMAX1(0.0,0.05*TFN3(NZ,NY,NX) &
      *(0.5*(ZPOOL(NB1(NZ,NY,NX),NZ,NY,NX)+ZPOOL(NB,NZ,NY,NX)) &
      -ZPOOL(NB,NZ,NY,NX)))
      XFRP=AMAX1(0.0,0.05*TFN3(NZ,NY,NX) &
      *(0.5*(PPOOL(NB1(NZ,NY,NX),NZ,NY,NX)+PPOOL(NB,NZ,NY,NX)) &
      -PPOOL(NB,NZ,NY,NX)))
      CPOOL(NB,NZ,NY,NX)=CPOOL(NB,NZ,NY,NX)+XFRC
      ZPOOL(NB,NZ,NY,NX)=ZPOOL(NB,NZ,NY,NX)+XFRN
      PPOOL(NB,NZ,NY,NX)=PPOOL(NB,NZ,NY,NX)+XFRP
...
!
      ...
      *AMAX1(0.0,WTRSVB(NB,NZ,NY,NX))
      XFRNX=FXFB(IBTYP(NZ,NY,NX)) &
      *AMAX1(0.0,WTRSBN(NB,NZ,NY,NX))*(1.0+CNR)
      XFRPX=FXFB(IBTYP(NZ,NY,NX)) &
      *AMAX1(0.0,WTRSBP(NB,NZ,NY,NX))*(1.0+CPR)
      ...
      ENDIF
      XFRCX=FXFB(IBTYP(NZ,NY,NX)) &
      *AMAX1(0.0,CPOOL(NB,NZ,NY,NX))
      XFRNX=FXFB(IBTYP(NZ,NY,NX)) &
      *AMAX1(0.0,ZPOOL(NB,NZ,NY,NX))*(1.0+CNL)
      XFRPX=FXFB(IBTYP(NZ,NY,NX)) &
      *AMAX1(0.0,PPOOL(NB,NZ,NY,NX))*(1.0+CPL)
      XFRC=AMIN1(XFRCX,XFRNX/CNMN,XFRPX/CPMN)
      XFRN=AMIN1(XFRNX,XFRC*CNMX,XFRPX*CNMX/CPMN*0.5)
      XFRP=AMIN1(XFRPX,XFRC*CPMX,XFRNX*CPMX/CNMN*0.5)
      CPOOL(NB,NZ,NY,NX)=CPOOL(NB,NZ,NY,NX)-XFRC
...
!     ENDIF
      ...
      -WTRSVB(NB,NZ,NY,NX)*WTRTRX)/WTPLTX
