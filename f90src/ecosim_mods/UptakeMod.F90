module UptakeMod
  ...
  include "blk11b.h"
      ...
      real(r8) :: CNH3B1,CNH3P1,CH2GS1,CH2GP1,CGSGL1,CHSGL1,CLSGL1
      real(r8) :: CQSGL1,CH4G1,CCO2S1,COXYS1,CCO2A1,COXYA1,CCH4A1
      real(r8) :: CZ2OA1,CNH3A1,CH2GA1,CCO2P1,COXYP1,COXYR,CO2PX
      real(r8) :: CH4PX,DTHS1,DIFFZ,DIFFU,DPSI,DIFH1P,DIFFL,DIFH1B
      real(r8) :: DIFH2P,DIFH2B,DIFNO3,DIFNOB,DIFNH4,DIFNHB,DIFOP
      real(r8) :: DFCOA,DFOXA,DFCHA,DFN2A,DFNHA,DFHGA,DFGP,DIFOL
      real(r8) :: DIFCL,DIFZL,DIFNL,DIFNB,DIFHL,DIFOX,DFGSP,EPX
      real(r8) :: EX,FPC,FTHRM,FDMR,FDTHS,FRADW,FRAD1,FRAD2,FNH3P
      real(r8) :: FP14X,FP1BX,FPO4X,FPOBX,FNO3X,FNOBX,FNH4X,FNHBX
      real(r8) :: HFLXS,H1POM,H1POX,H1PXM,H1PXB,H2POM,H2POX,H2PXM
      real(r8) :: H2PXB,H2GA1,H2GP1,H2GS1,HGSGL1,HLSGL1,H2GG1,H2GPX
      real(r8) :: OSTRN,OSTRD,OSWT,OXYA1,OXYP1,OXYG1,OXYS1,OGSGL1
      real(r8) :: OLSGL1,OLSGLP,OXYPX,PAREX,PARHX,PSIL2,PAREC,PARHC
      real(r8) :: PSILC,POSGX,PATHL,PPOOLX,RTDPZ,RTDPX,RA1,RSSZ
      real(r8) :: RSSUX,RSSU,RSSL,RTAR2,RTK,RMFH1P,RTKH1P,RTKHP1
      real(r8) :: RMFH2B,RTKH1B,RTKHB1,RMFH2P,RTKH2P,RTKHPP,RTKH2B
      real(r8) :: RTKHPB,RMFNO3,RTKNO3,RTKNOP,RMFNOB,RTKNOB,RTKNPB
      real(r8) :: RMFNH4,RTKNH4,RTKNHP,RMFNHB,RTKNHB,RTKNBP,RTVLWA
      ...
      real(r8) :: WVPLT,WFNC,XC,X,XFRC,XFRN,XFRP,Y,ZPOOLB,ZOSGX
      real(r8) :: ZNO3M,ZNO3X,ZNOBM,ZNOBX,ZNSGX,ZNH4M,ZNH4X,ZNHBM
      ...
      ,RSRT(2,JZ),RSRG(2,JZ),RSR1(2,JZ),RSR2(2,JZ) &
      ,RSSX(2,JZ),RSRS(2,JZ),WTRTG(JZ),FPQ(2,JZ,05),FPP(2,JZ,05) &
      ,RACZ(JP,JY,JX),FRTDPX(JZ,05),RTARR(2,JZ) &
           ,VOLPU(JZ),VOLWU(JZ)
...
!     WRITE(*,2123)'START',I,J,NX,NY,NZ
!    2,IDAY(1,NB1(NZ,NY,NX),NZ,NY,NX),ARLFS(NZ,NY,NX),PP(NZ,NY,NX)
!    3,FRADP(NZ,NY,NX),RTDP1(1,1,NZ,NY,NX),RADC(NZ,NY,NX)
!    4,RADP(NZ,NY,NX),FRTDPX(L,NZ),RTDPX,RAZ(NZ,NY,NX)
...
!     WRITE(*,4411)'RAC',I,J,NZ,RACZ(NZ,NY,NX),RAB(NY,NX),RAZ(NZ,NY,NX)
!    2,ZT(NY,NX),ZD(NY,NX),ZR(NY,NX),ZC(NZ,NY,NX),ALFZ
!    3,TFRADP,RACX,EXP(-ALFZ*ZC(NZ,NY,NX)/ZT(NY,NX))
!    4,EXP(-ALFZ*(ZD(NY,NX)+ZR(NY,NX))/ZT(NY,NX)),RADP(NZ,NY,NX)
!    5,FRADP(NZ,NY,NX),DPTHS(NY,NX)
...
2005  CONTINUE
      ...
      ENDIF
      ENDIF
!     IF(NZ.EQ.1.OR.NZ.EQ.2)THEN
!     WRITE(*,4413)'FRTDPX',I,J,NZ,L,N,FRTDPX(L,NZ)
!    2,RTDPX,DLYR(3,L,NY,NX),RTDPZ,RTDP1(1,1,NZ,NY,NX)
!    3,SDPTH(NZ,NY,NX),CDPTHZ(L-1,NY,NX),HTCTL(NZ,NY,NX)
!    4,WTRTD(1,L,NZ,NY,NX),WTRTG(L),FPQ(N,L,NZ)
!     ENDIF
      ENDIF
      ...
      .AND.FRTDPX(L,NZ).GT.ZERO)THEN
      RRADL(N,L)=AMAX1(RRAD2X(N,NZ,NY,NX),SQRT((RTVLW(N,L,NZ,NY,NX) &
      /(1.0-PORT(N,NZ,NY,NX)))/(3.1416*PP(NZ,NY,NX) &
      *RTLGP(N,L,NZ,NY,NX))))
      PATH(N,L)=AMAX1(1.001*RRADL(N,L) &
      ,1.0/(SQRT(3.1416*(RTDNP(N,L,NZ,NY,NX)/FRTDPX(L,NZ)) &
      /FMPR(L,NY,NX))))
      ...
      ENDIF
!     IF(NZ.EQ.1.OR.NZ.EQ.2)THEN
!     WRITE(*,4413)'RTAR',I,J,NX,NY,NZ,L,N
!    2,RTARR(N,L),RRADL(N,L),PATH(N,L),WTRTD(N,L,NZ,NY,NX)
!    2,RTLGP(N,L,NZ,NY,NX),RTDNP(N,L,NZ,NY,NX)
!    2,PP(NZ,NY,NX),RTDNP(N,L,NZ,NY,NX)*PP(NZ,NY,NX)
!    2/AREA(3,L,NY,NX),FRTDPX(L,NZ),RTLG2X(N,NZ,NY,NX)
!    2,FPQ(N,L,NZ),WTRTG(L)
!    3,RRADL(N,L),RRAD2X(N,NZ,NY,NX),RTVLW(N,L,NZ,NY,NX)
!    2,PORT(N,NZ,NY,NX),PP(NZ,NY,NX),RTLGP(N,L,NZ,NY,NX)
...
!     WRITE(*,4443)'RI',I,J,NX,NY,NZ,NN,RI,RIB(NY,NX)
!    2,TKA(NY,NX),TKC1
...
!
      ...
      *VOLA(L,NY,NX)/VOLWM(NPH,L,NY,NX)
!
...
!
      ...
      /(FRAD1*RTN1(1,L,NZ,NY,NX)) &
      +RSRA(1,NZ,NY,NX)*HTSTZ(NZ,NY,NX) &
      /(FRADW*RTN1(1,L,NZ,NY,NX))
      FRAD2=(RRAD2(N,L,NZ,NY,NX)/RRAD2M(N,NZ,NY,NX))**4
      RSR2(N,L)=RSRA(N,NZ,NY,NX)*RTLGA(N,L,NZ,NY,NX) &
      /(FRAD2*RTNL(N,L,NZ,NY,NX))
      ELSE
      ILYR(N,L)=0
      ENDIF
      enddo
...
!
      ...
      ,AMAX1((CNH3E(NY,NX)-CNH3P)/(RA(NZ,NY,NX)+RC(NZ,NY,NX)) &
      *FRADP(NZ,NY,NX)*AREA(3,NU(NY,NX),NY,NX) &
      *ARLFB(NB,NZ,NY,NX)/ARLFP(NZ,NY,NX),-0.1*ZPOOLB))
      ELSE
      RNH3B(NB,NZ,NY,NX)=0.0
      ENDIF
!     WRITE(*,7777)'RNH3',I,J,NZ,NB,RNH3B(NB,NZ,NY,NX)
!    2,RNH3C(NZ,NY,NX),CNH3E(NY,NX),CNH3P,RA(NZ,NY,NX),RC(NZ,NY,NX)
!    2,ARLFB(NB,NZ,NY,NX),ARLFP(NZ,NY,NX),SNH3P
!    4,ZPOOL(NB,NZ,NY,NX),WTLSB(NB,NZ,NY,NX),FRADP(NZ,NY,NX)
...
!
      ...
      -ZPOOLX*OQC(K,L,NY,NX))/CPOOLT
      XFRP=(OQP(K,L,NY,NX)*CPOOLR(N,L,NZ,NY,NX) &
      -PPOOLX*OQC(K,L,NY,NX))/CPOOLT
...
!     WRITE(*,8766)'XOQCSU',I,J,NX,NY,L,NZ,K,N
!    2,XOQCS(K,L,NY,NX),RDFOMC(N,K,L,NZ,NY,NX)
...
!
      ...
      .AND.FPQ(N,L,NZ).GT.ZEROQ(NZ,NY,NX))THEN
...
!
      ...
      ,AMAX1(0.0,OXYP1/RTVLW(N,L,NZ,NY,NX)))
      CCH4P1=AMAX1(0.0,CH4P1/RTVLW(N,L,NZ,NY,NX))
      CN2OP1=AMAX1(0.0,Z2OP1/RTVLW(N,L,NZ,NY,NX))
      CNH3P1=AMAX1(0.0,ZH3P1/RTVLW(N,L,NZ,NY,NX))
      CH2GP1=AMAX1(0.0,H2GP1/RTVLW(N,L,NZ,NY,NX))
...
!     C*P1=root aqueous concentration
!
!     IF(IYRC.EQ.2000.AND.I.LE.180)THEN
!     WRITE(*,5555)'COXYR',I,J,NX,NY,NZ,L,N,M,MX,COXYR,RUPOXR
!    2,RMFOXS,RDFOXS,RDFOXP,COXYS1,COXYS1-COXYR,COXYP1,FOXYX
!    3,WTRTG(L),DIFOL,DIFOP,THETM,OLSGL1,UPWTRH,RTARR(N,L)
!    5,RTARRX,UPMXP,THETW(L,NY,NX),OXYS1,OXYS(L,NY,NX),OXYP1
!    3,OXYP(N,L,NZ,NY,NX),ROXYY(L,NY,NX),RTLGP(N,L,NZ,NY,NX)
!    2,UPMXP,DIFOX,THETW1,THETM,RRADS,FPQ(N,L,NZ)
!    4,RUPOXS(N,L,NZ,NY,NX),RUPOXP(N,L,NZ,NY,NX)
!    5,COXYE(NY,NX),SOXYL(L,NY,NX),FRTDPX(L,NZ)
...
!     IF(NZ.EQ.1.AND.L.EQ.7.AND.N.EQ.1)THEN
!     WRITE(*,5547)'RCO2S',I,J,NX,NY,NZ,L,N,RCO2S(N,L,NZ,NY,NX)
!    2,RCO2SX,RDXCOS,RDFCOS,RMFCOS,DIFCL,CCO2S1,CCO2P1,VOLWSP,VOLWMM
!     WRITE(*,5547)'RCH4S',I,J,NX,NY,NZ,L,N,RUPCHS(N,L,NZ,NY,NX)
!    2,RUPCSX,RDXCHS,RDFCHS,RMFCHS,DIFCL,CCH4S1,CCH4P1,VOLWSP,VOLWMM
!     WRITE(*,5547)'RUPNBX',I,J,NX,NY,NZ,L,N,RUPNBX,RDXNHB,RDFN3B
!    2,RTVLWB,ZH3B1,VOLWMB,ZH3PB,VOLWSB,RUPN3B(N,L,NZ,NY,NX)
!    3,RNBDFQ,RNHDF1,RUPNSX
!     WRITE(*,5547)'RUPN2S',I,J,NX,NY,NZ,L,N,RUPN2S(N,L,NZ,NY,NX)
!    2,RUPZSX,RDXN2S,RDFN2S,RTVLW(N,L,NZ,NY,NX),Z2OS1,Z2OP1,VOLWMM
!    2,RMFN2S,DIFZL,CN2OS1,CN2OP1,PP(NZ,NY,NX)
...
!     IF(NZ.EQ.1.AND.L.EQ.7.AND.N.EQ.1)THEN
!     WRITE(*,5566)'CO2P1',I,J,NX,NY,NZ,L,N,M,MX,RCO2S(N,L,NZ,NY,NX)
!    2,RCO2SX,RDFCOS,RDXCOS,RMFCOS,DIFCL,DFGS(M,L,NY,NX)
!    3,CCO2S1,CCO2P1,RTVLW(N,L,NZ,NY,NX),CO2S1,VOLWMM
!    4,CO2P1,VOLWSP,PP(NZ,NY,NX),FPQ(N,L,NZ),RCODF1,RCO2PX
!    5,CO2PX,RTVLP(N,L,NZ,NY,NX),DFGP,VOLWCA,CO2A1
!    6,XNPD,PORT(N,NZ,NY,NX),TFND(L,NY,NX),RCO2FX
!    7,RCODFQ,DFGSP,FOXYX,CO2G1,VOLWCO,VOLPMM,THETPM(M,L,NY,NX),THETX
!    8,ROXYP(N,L,NZ,NY,NX),ROXYY(L,NY,NX)
!     WRITE(*,5566)'OXYP1',I,J,NX,NY,NZ,L,N,M,MX,UPMXP*PP(NZ,NY,NX)
!    2,RUPOSX,ROXDFQ,OXYS1,RUPOPX,ROXDF1,OXYP1
!    3,FOXYX,DFGS(M,L,NY,NX),DFGP,ROXYFX,ROXYLX,ROXFL1
!    3,OXYG1,OXYA1,COXYS1,COXYP1,COXYR,ROXSK(M,L,NY,NX),XS,XR
!    4,OXYPY,VOLWOA,RTVLP(N,L,NZ,NY,NX),RTVLW(N,L,NZ,NY,NX)
!    5,DFOXA,COXYE(NY,NX),COXYA1,RUPOXP(N,L,NZ,NY,NX)
!    6,RUPOXS(N,L,NZ,NY,NX),ROXYP(N,L,NZ,NY,NX),THETPM(M,L,NY,NX)
!     WRITE(*,5566)'CH4S1',I,J,NX,NY,NZ,L,N,M,MX,CH4S1,RCHDFQ,RUPCSX
!    2,RDFCHS,RMFCHS,DIFCL,CCH4S1,CCH4P1,CH4P1,CH4PX,RCHDF1,RCHFL1
!    3,DFCHA,RTVLP(N,L,NZ,NY,NX),CCH4E(NY,NX),CCH4A1,THETM,CQSGL1,RTARRX
!    4,RRADS,FILM(M,L,NY,NX),RRADL(N,L),RRAD2M(N,NZ,NY,NX)
!    5,RTDNP(N,L,NZ,NY,NX)
!    4,DFGS(M,L,NY,NX),CH4G1,VOLWCH,CH4S1,VOLPMM,THETPM(M,L,NY,NX)
...
!     IF(NZ.EQ.1.OR.NZ.EQ.4)THEN
!     WRITE(*,3368)'WFR',I,J,NX,NY,NZ,L,N,WFR(N,L,NZ,NY,NX)
!    2,RUPOXP(N,L,NZ,NY,NX),RUPOXS(N,L,NZ,NY,NX)
!    3,ROXYP(N,L,NZ,NY,NX)
...
!
      ...
      .AND.FCUP.GT.ZERO.AND.FWSRT.GT.ZERO &
      .AND.RTLGP(N,L,NZ,NY,NX).GT.ZEROP(NZ,NY,NX))THEN
!
