...
!
        DO  N=1,NumMicbFunGroups
          DO NGL=JGniA(N),JGnfA(N)
            DO M=1,nlbiomcp
              MID=micpar%get_micb_id(M,NGL)
              OCH=DCORPC*OMCff(MID,L,NY,NX)
              ONH=DCORPC*OMNff(MID,L,NY,NX)
              OPH=DCORPC*OMPff(MID,L,NY,NX)
              ONX=EFIRE(1,ITILL(I,NY,NX))*ONH
              OPX=EFIRE(2,ITILL(I,NY,NX))*OPH
              ONL(4,1)=ONL(4,1)+ONH-ONX
              OPL(4,1)=OPL(4,1)+OPH-OPX
              OMCff(MID,L,NY,NX)=OMCff(MID,L,NY,NX)-OCH
              OMNff(MID,L,NY,NX)=OMNff(MID,L,NY,NX)-ONH
              OMPff(MID,L,NY,NX)=OMPff(MID,L,NY,NX)-OPH
              DC=DC+OMCff(MID,L,NY,NX)
              DN=DN+OMNff(MID,L,NY,NX)
              DP=DP+OMPff(MID,L,NY,NX)
              OC=OC+OCH
              ON=ON+ONX
              OP=OP+OPX
            enddo
