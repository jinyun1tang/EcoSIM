...
=WTSHT(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !canopy shoot C, SHOOT_C
=WTLF(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !canopy leaf C, LEAF_C
=WTSHE(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !canopy sheath C, SHTH_C
=WTSTK(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !STALK_C
=WTRSV(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !RESERVE_C
=(WTHSK(NZ,NY,NX)+WTEAR(NZ,NY,NX))/AREA(3,NU(NY,NX),NY,NX) !REPRO_C, reproduction C
=WTGR(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !GRAIN_C
=WTRT(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !ROOT_C
=WTND(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !NDL_C, nodule C
=WTRVC(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !STORED_C
...
=TCUPTK(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX) !EXUD_C, plant C exudation, gC/m2
=TCSNC(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !LITTER_C, total plant litterfall C, gC/m2
=TCSN0(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !SF_LIT_C, plant literfall to surface, gC/m2
...
=CCPOLP(NZ,NY,NX)                         ![SOL._C], canopy nonstructural C concentration, gC/m2
=HVSTC(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !HVST_C
=RTDNP(1,1:JZ,NZ,NY,NX)*PP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !DNS_RT, root layer length density, m m^-3

=BALC(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !C_BALANCE
=WTSTG(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX) !STG_DEAD_C
...
!
=WTSHN(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)    !SHOOT_N
=WTLFN(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)    !LEAF_N
=WTSHEN(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !SHTH_N
=WTSTKN(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !STALK_N
=WTRSVN(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !RESERVE_N
=(WTHSKN(NZ,NY,NX)+WTEARN(NZ,NY,NX))/AREA(3,NU(NY,NX),NY,NX)  !HUSK_N
=WTGRNN(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !GRAIN_N
=WTRTN(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)    !ROOT_N
=WTNDN(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)    !NDL_N
=WTRVN(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)    !STORED_N
=TZUPTK(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !UPTK_N
=TZSNC(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)    !LITTER_N
=TZUPFX(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !TL_N_FIXED

=CZPOLP(NZ,NY,NX)   ![SOL_N]
=(WTLFN(NZ,NY,NX)+ZPOOLP(NZ,NY,NX))/(WTLF(NZ,NY,NX)+CPOOLP(NZ,NY,NX))  !N_STRESS, NC ratio

=CPPOLP(NZ,NY,NX)  ![SOL_P]

=(WTLFP(NZ,NY,NX)+PPOOLP(NZ,NY,NX))/(WTLF(NZ,NY,NX)+CPOOLP(NZ,NY,NX))  !'P_STRESS', PC ratio

=TNH3C(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !NH3_FLUX
=HVSTN(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !HVST_N
=BALN(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)    !N_BALANCE
=WTSTGN(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !STG_DEAD_N
=VNH3F(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !FIRE_N
=TZSN0(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !SF_LIT_N

=WTSHP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !SHOOT_P
=WTLFP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !LEAF_P
=WTSHEP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !SHTH_P
=WTSTKP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !STALK_P
=WTRSVP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !RESERVE_P
=(WTHSKP(NZ,NY,NX)+WTEARP(NZ,NY,NX))/AREA(3,NU(NY,NX),NY,NX)  !HUSK_P
=WTGRNP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !GRAIN_P
=WTRTP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !ROOT_P
=WTNDP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !NDL_P, nodule P
=WTRVP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !STORED_P
=TPUPTK(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !UPTK_P
=TPSNC(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !LITTER_P
=CPPOLP(NZ,NY,NX)                          ![SOL_P]

cancp(NZ,NY,NX)=(WTLFP(NZ,NY,NX)+PPOOLP(NZ,NY,NX))/(WTLF(NZ,NY,NX)+CPOOLP(NZ,NY,NX))  !P_STRESS

=HVSTP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !HVST_P
=BALP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !P_BALANCE
WTSTGPXY(NZ,NY,NX)=WTSTGP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !STG_DEAD_P
VPO4FXY(NZ,NY,NX)=VPO4F(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !FIRE_P
TPSN0XY(NZ,NY,NX)=TPSN0(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)  !SF_LIT_P, surface P litterfall
!
...
CANNC(NZ,NY,NC)=(WTLFN(NZ,NY,NX)+ZPOOLP(NZ,NY,NX))/(WTLF(NZ,NY,NX)+CPOOLP(NZ,NY,NX))  !LEAF_N/C

CANPC(NZ,NY,NX)=(WTLFP(NZ,NY,NX)+PPOOLP(NZ,NY,NX))/(WTLF(NZ,NY,NX)+CPOOLP(NZ,NY,NX))  !LEAF_P/C
...
XHVSTCXY(NY,NX)=XHVSTC(NY,NX)/AREA(3,NU(NY,NX),NY,NX)
...
XHVSTNXY(NY,NX)=XHVSTN(NY,NX)/AREA(3,NU(NY,NX),NY,NX)    !ECO_HVST_N
...
XHVSTPXY(NY,NX)=XHVSTP(NY,NX)/AREA(3,NU(NY,NX),NY,NX)   !ECO_HVST_P
