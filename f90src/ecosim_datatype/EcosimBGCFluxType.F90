...
! Ecosystm fluxes for C, N, and P budget
  ...
  real(r8),target,allocatable ::  LitrFallChemElm_col(:,:,:)                       !total LitrFall element, [g d-2 h-1]
...
contains
  ...
  allocate(LitrFallChemElm_col(NumPlantChemElms,JY,JX));       LitrFallChemElm_col=0._r8
...
!----------------------------------------------------------------------
  ...
  call destroy(LitrFallChemElm_col)
