...
!------------------------------------------------------------------------------------------
  ...
  subroutine SoluteMicporeGainXBoundaryM(M3,M2,M1,M,N,M6,M5,M4)
!
!     Description:
!
  implicit none
...
!------------------------------------------------------------------------------------------
  ...
  subroutine AccumXBoundaryFluxM(N,M6,M5,M4)
!
!     Description:
!
  implicit none
...
!
  ...
  ENDDO
  end subroutine AccumXBoundaryFluxM
!------------------------------------------------------------------------------------------
            ...
            IF(VLSoilPoreMicP_vr(N3,N2,N1).GT.ZEROS(NY,NX))THEN
              ...
              IF(FlowDirIndicator(M2,M1).NE.3 .OR. N.EQ.iVerticalDirection)THEN
                ...
                ELSE
                  ...
                  !
                  call SoluteMicporeGainXBoundaryM(M3,M2,M1,M,N,M6,M5,M4)
                ENDIF
                !
...
!
                !  ACCUMULATE HOURLY FLUXES FOR USE IN REDIST.F
                call AccumXBoundaryFluxM(N,M6,M5,M4)
              ENDIF
            ENDIF
          ENDDO D9575
          !
...
!
  ...
  D9695: DO NX=NHW,NHE
    D9690: DO NY=NVN,NVS
      ...
      DO idsalt=idsalt_beg,idsalt_end
        trcSalt_ml2_snvr(idsalt,1,NY,NX)=trcSalt_ml2_snvr(idsalt,1,NY,NX)+trcSalt_SnowDrift_flxM_col(idsalt,NY,NX)
      ENDDO

      D9670: DO L=1,JS
        DO idsalt=idsalt_beg,idsalt_end
          trcSalt_ml2_snvr(idsalt,L,NY,NX)=trcSalt_ml2_snvr(idsalt,L,NY,NX)+trcSalt_Aqua_flxM_snvr(idsalt,L,NY,NX)
        ENDDO
      ENDDO D9670
