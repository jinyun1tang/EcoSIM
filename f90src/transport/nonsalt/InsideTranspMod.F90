...
!
    CCO2G2=AMAX1(0.0,CO2G2(NU(NY,NX),NY,NX)/VOLPM(M,NU(NY,NX),NY,NX))
    CCH4G2=AMAX1(0.0,CH4G2(NU(NY,NX),NY,NX)/VOLPM(M,NU(NY,NX),NY,NX))
    COXYG2=AMAX1(0.0,OXYG2(NU(NY,NX),NY,NX)/VOLPM(M,NU(NY,NX),NY,NX))
    CZ2GG2=AMAX1(0.0,Z2GG2(NU(NY,NX),NY,NX)/VOLPM(M,NU(NY,NX),NY,NX))
    CZ2OG2=AMAX1(0.0,Z2OG2(NU(NY,NX),NY,NX)/VOLPM(M,NU(NY,NX),NY,NX))
    CNH3G2=AMAX1(0.0,ZN3G2(NU(NY,NX),NY,NX)/VOLPM(M,NU(NY,NX),NY,NX))
    CH2GG2=AMAX1(0.0,H2GG2(NU(NY,NX),NY,NX)/VOLPM(M,NU(NY,NX),NY,NX))
!
!     EQUILIBRIUM CONCENTRATIONS AT SOIL SURFACE AT WHICH
...
!
    IF(FLQM(3,NU(NY,NX),NY,NX).GT.0.0)THEN
      IF(VOLPM(M,NU(NY,NX),NY,NX).GT.ZEROS2(NY,NX))THEN
        VFLW=-AMAX1(0.0,AMIN1(VFLWX,FLQM(3,NU(NY,NX),NY,NX)/VOLPM(M,NU(NY,NX),NY,NX)))
      ELSE
        VFLW=-VFLWX
      ENDIF
...
!
    IF(VOLWM(M,NU(NY,NX),NY,NX).GT.ZEROS2(NY,NX))THEN
      ...
      IF(VOLN3T(NY,NX).GT.ZEROS2(NY,NX).AND.VOLWXA(NU(NY,NX),NY,NX).GT.ZEROS2(NY,NX))THEN
        ...
        CNH4S0=AMAX1(0.0,ZNH4S2(NU(NY,NX),NY,NX))/VOLWXA(NU(NY,NX),NY,NX)
      ELSE
      ...
      IF(VOLNBT(NY,NX).GT.ZEROS2(NY,NX) &
        ...
        CNH4B0=AMAX1(0.0,ZNH4B2(NU(NY,NX),NY,NX))/VOLWXB(NU(NY,NX),NY,NX)
      ELSE
...
!
            IF(FLWM(M,N,N6,N5,N4).GT.0.0)THEN
              IF(VOLWM(M,N3,N2,N1).GT.ZEROS2(N2,N1))THEN
                VFLW=AMAX1(0.0,AMIN1(VFLWX,FLWM(M,N,N6,N5,N4)/VOLWM(M,N3,N2,N1)))
              ELSE
                VFLW=VFLWX
              ENDIF
              DO  K=0,jcplx1
...
!
      ...
      SCO2X=12.0*SCO2L(N3,N2,N1)  !conver into carbon g C/mol
