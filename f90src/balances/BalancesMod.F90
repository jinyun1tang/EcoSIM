...
!------------------------------------------------------------------------------------------
  ...
  DO  NX=NHW,NHE
    DO  NY=NVN,NVS
      WaterErr_col(NY,NX)=WaterErr_col(NY,NX)-WatMass_col(NY,NX)+(RainFalPrec(NY,NX)+SnoFalPrec(NY,NX)) &
        +VapXAir2GSurf_col(NY,NX)+QvET_col(NY,NX)-QDischar_col(NY,NX)+QRunSurf_col(NY,NX)
    ENDDO
  ENDDO
