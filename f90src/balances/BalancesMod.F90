...
!------------------------------------------------------------------------------------------
  ...
  DO  L=1,JS
    ...
    WatMass_col(NY,NX) = WatMass_col(NY,NX)+ VLDrySnoWE_snvr(L,NY,NX)+VLWatSnow_snvr(L,NY,NX)+VLIceSnow_snvr(L,NY,NX)*DENSICE
  ENDDO
  
  WatMass_col(NY,NX) = WatMass_col(NY,NX)+ WatHeldOnCanopy_col(NY,NX)+CanopyWat_col(NY,NX)+VLWatMicP_vr(0,NY,NX)+VLiceMicP_vr(0,NY,NX)*DENSICE
  ...
  DO  NX=NHW,NHE
    DO  NY=NVN,NVS
      ...
      WaterErr_test = WaterErr_col(NY,NX)-WatMass_col(NY,NX)+ PrecAtm_col(NY,NX)+Irrigation_col(NY,NX) &
        ...
        -QDrain_col(NY,NX)-QDischar_col(NY,NX)-QCanopyWat2Dist_col(NY,NX)
