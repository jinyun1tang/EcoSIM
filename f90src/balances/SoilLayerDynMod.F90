...
implicit none
  ...
  call SoilRelayering(NY,NX,DORGC,DVLiceMicP,CDPTHX,CDPTHY,IFLGL)
...
!------------------------------------------------------------------------------------------
  subroutine SoilRelayering(NY,NX,DORGC,DVLiceMicP,CDPTHX,CDPTHY,IFLGL)
!
  ...
  D225: DO LX=NL(NY,NX),NU(NY,NX),-1
    ...
    IF(SoiBulkDensity_vr(LX,NY,NX).LE.ZERO)THEN  !current layer is water
      ...
      IF(SoiBulkDensity_vr(LX+1,NY,NX).GT.ZERO .OR. itoplyr_type.EQ.ist_soil)THEN        
        DLYR_XMicP=DLYR(3,LX,NY,NX)-(VLWatMicP_vr(LX,NY,NX)+VLiceMicP(LX,NY,NX))/AREA(3,LX,NY,NX)
      ...
      ELSE        
        ...
        DLYR_XMicP=DLYRI(3,LX,NY,NX)-(VLWatMicP_vr(LX,NY,NX)+VLiceMicP(LX,NY,NX))/AREA(3,LX,NY,NX)
    ...
    DO  NZ=1,NP(NY,NX)
      IF(RootMycoActiveBiomC_pvr(ipltroot,L0,NZ,NY,NX).GT.ZERO4Groth_pft(NZ,NY,NX) &
        ...
        DO NE=1,NumPlantChemElms
          ...
          RootNodulNonstElms_pvr(NE,L1,NZ,NY,NX)=RootNodulNonstElms_pvr(NE,L1,NZ,NY,NX)+FX*RootNodulNonstElms_pvr(NE,L0,NZ,NY,NX)
        ENDDO
      ENDIF
    ENDDO
  ENDIF
    ...
    DO  NZ=1,NP(NY,NX)
      IF(RootMycoActiveBiomC_pvr(ipltroot,L0,NZ,NY,NX).GT.ZERO4Groth_pft(NZ,NY,NX) &
        ...
        DO NE=1,NumPlantChemElms
          ...
          RootNodulNonstElms_pvr(NE,L0,NZ,NY,NX)=FY*RootNodulNonstElms_pvr(NE,L0,NZ,NY,NX)
        ENDDO
      ENDIF
    ENDDO
  ENDIF
...
!
        DO NE=1,NumPlantChemElms
          WTNDLE=FRO*RootNodulStrutElms_pvr(NE,L0,NZ,NY,NX)
          RootNodulStrutElms_pvr(NE,L1,NZ,NY,NX)=RootNodulStrutElms_pvr(NE,L1,NZ,NY,NX)+WTNDLE
          RootNodulStrutElms_pvr(NE,L0,NZ,NY,NX)=RootNodulStrutElms_pvr(NE,L0,NZ,NY,NX)-WTNDLE

          FXEPOOLN=FRO*RootNodulNonstElms_pvr(NE,L0,NZ,NY,NX)
          RootNodulNonstElms_pvr(NE,L1,NZ,NY,NX)=RootNodulNonstElms_pvr(NE,L1,NZ,NY,NX)+FXEPOOLN
          RootNodulNonstElms_pvr(NE,L0,NZ,NY,NX)=RootNodulNonstElms_pvr(NE,L0,NZ,NY,NX)-FXEPOOLN
