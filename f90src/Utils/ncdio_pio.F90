...
! check dimension, check variable
  ...
  public :: ncd_init
  public :: ncd_getvint
  public :: ncd_inq_varid      ! function to inquire variale id
  ...
  interface ncd_putvar
    ...
    module procedure ncd_putvar_string_1d
  ...
  interface ncd_putatt
     ...
     module procedure ncd_putatt_int_1d
  ...
  interface ncd_io
     ...
     module procedure ncd_io_4d_double_glob
     !TYPE text
     !DIMS 0,1,2
     ...
     module procedure ncd_io_5d_double
     !TYPE logical
     !DIMS 1
...
!-----------------------------------------------------------------------
  ...
  if (present(missing_value)) then
    call check_ret(ncd_putatt(ncid, varid, 'missing_value', missing_value,lxtype),subname)
  end if

  if (present(ifill_value)) then
    call check_ret(ncd_putatt(ncid, varid, '_FillValue', ifill_value),subname)
  end if
  ...
  if (present(flag_meanings)) then
      ...
      call check_ret(ncd_putatt(ncid,varid,'flag_meanings', trim(str)),subname)
  end if

  if (present(imissing_value)) then
    call check_ret(ncd_putatt(ncid, varid, 'missing_value', imissing_value),subname)
  end if

  if (present(nvalid_range)) then
    call check_ret(ncd_putatt(ncid,varid,'valid_range', nvalid_range ),subname)
  end if
...
!**********************************************************************
  ...
  call check_ret( nf90_put_var(ncid%fh, vardesc%varid, data,  &
    start = (/rec/)),'ncd_putvar_real_sp '//trim(varname))
...
!**********************************************************************
  ...
  call check_ret( nf90_put_var(ncid%fh, vardesc%varid, data),'ncd_putvar_int_all_1d')

  end subroutine ncd_putvar_int_all_1d
...
!----------------------------------------------------------------------
  ...
  implicit none
    ...
    character(len=*), parameter :: sub=trim(mod_filename)//'::ncd_putvar_string_1d'
    integer :: varid
    logical :: readvar
    type(Var_desc_t)  :: vardesc
...
!----------------------------------------------------------------------
  ...
  implicit none
    ...
    character(len=*), parameter :: sub=trim(mod_filename)//'::ncd_getvar_string_1d'
    integer :: varid
    logical :: readvar
    type(Var_desc_t)  :: vardesc

    print*,'varname ',varname, len(data)
    call check_var(ncid, trim(varname), vardesc, readvar)

    call check_ret( nf90_get_var(ncid%fh, vardesc%varid, data,start=(/1,rec/),&
      count = (/len(data),1/)),trim(sub))

  end subroutine ncd_getvar_string_1d
  ...
  !**********************************************************************
    ...
    call check_ret( nf90_put_var(ncid%fh, vardesc%varid, data),&
      trim(mod_filename)//'::ncd_putvar_real_sp_all_2d::'//trim(varname))
...
!**********************************************************************
  ...
  subroutine ncd_putvar_int_all_2d(ncid, varname, data)
  !
  ! DESCRIPTION
  ! put 2d integer array to file
...
!**********************************************************************
  ...
  call check_ret( nf90_put_var(ncid%fh, vardesc%varid, data),'ncd_putvar_int_all_2d')

  end subroutine ncd_putvar_int_all_2d
!----------------------------------------------------------------------
...
!**********************************************************************
  ...
  call check_ret( nf90_put_var(ncid%fh, vardesc%varid, data,  &
   start = (/1,1,1,rec/)),'ncd_putvar_int_3d')

  end subroutine ncd_putvar_int_3d
!----------------------------------------------------------------------

  subroutine ncd_putvar_int_all_3d(ncid, varname, data)
  !
  ! DESCRIPTION
  ! put 3d integer to file
  !
...
!**********************************************************************
  ...
  end subroutine ncd_putvar_int_all_3d
!----------------------------------------------------------------------
...
!----------------------------------------------------------------------
  subroutine ncd_putvar_real_sp_all_5d(ncid,varname,data)
...
!----------------------------------------------------------------------
  subroutine ncd_putvar_real_sp_4d(ncid, varname, rec, data)
  !
  ! DESCRIPTION
  ! put 3d real array to file
...
!**********************************************************************
  ...
  call check_ret( nf90_put_var(ncid%fh, vardesc%varid, data,  &
     start = (/1,1,1,1,rec/)),trim(varname)//' in ncd_putvar_real_sp_4d')

  end subroutine ncd_putvar_real_sp_4d
!----------------------------------------------------------------------
  subroutine ncd_putvar_real_sp_5d(ncid, varname, rec, data)
  !
  ! DESCRIPTION
  ! put 3d real array to file
...
!**********************************************************************
  ...
  call check_ret( nf90_put_var(ncid%fh, vardesc%varid, data,  &
     start = (/1,1,1,1,rec/)),trim(varname)//' in ncd_putvar_real_sp_5d')

  end subroutine ncd_putvar_real_sp_5d
!----------------------------------------------------------------------
...
!**********************************************************************
   ...
   !**********************************************************************
    ...
    !**********************************************************************
      ...
      !**********************************************************************
        ...
        logical :: readvar
        type(Var_desc_t)  :: vardesc
        character(len=*), parameter :: subname=mod_filename//'::ncd_getvar_real_sp_all_3d'
        call check_var(ncid, trim(varname), vardesc, readvar)
  ...
  !**********************************************************************
    ...
    logical :: readvar
    type(Var_desc_t)  :: vardesc
    character(len=*), parameter :: subname=mod_filename//'::ncd_getvar_real_sp_all_4d'
    call check_var(ncid, trim(varname), vardesc, readvar)
  ...
  !----------------------------------------------------------------------
    ...
    subroutine ncd_getvar_real_sp_all_5d(ncid, varname, data)
    !
    !DESCRIPTION
    ! read a real scalar
  ...
  !**********************************************************************
    ...
    call check_ret( nf90_get_var(ncid%fh, vardesc%varid, data),'ncd_getvar_real_sp_all_5d')

    end subroutine ncd_getvar_real_sp_all_5d

!----------------------------------------------------------------------

  subroutine ncd_getvar_real_sp(ncid, varname, rec, data,readvar)
  !
  !DESCRIPTION
  ! read a real scalar
...
!**********************************************************************
  ...
  logical :: lreadv
  type(Var_desc_t)  :: vardesc
  character(len=*), parameter :: subname=mod_filename//'::ncd_getvar_real_sp'
...
!----------------------------------------------------------------------
  ...
  subroutine ncd_getvar_real_sp_scalar(ncid, varname,  data, readvar)
  !
  !DESCRIPTION
  ! read a real scalar
...
!**********************************************************************
  ...
  logical :: lreadv
  type(Var_desc_t)  :: vardesc
  character(len=*), parameter :: subname=mod_filename//'::ncd_getvar_real_sp_scalar'
  call check_var(ncid, trim(varname), vardesc, lreadv)
  if(present(readvar))readvar=lreadv
...
!----------------------------------------------------------------------
  subroutine ncd_getvar_int_all_2d(ncid, varname, data)
  !
  ! DESCRIPTION
  ! read 2d integer array
  !
...
!----------------------------------------------------------------------
  subroutine ncd_getvar_int_all_3d(ncid, varname, data)
  !
  ! DESCRIPTION
  ! read 3d integer array
...
!----------------------------------------------------------------------
  subroutine ncd_getvar_real_sp_4d(ncid, varname, rec, data)
  !
  ! DESCRIPTION
  ! read 3d real array
...
!**********************************************************************
  ...
  call check_ret( nf90_get_var(ncid%fh, vardesc%varid, data,  &
    start = (/1,1,1,1,rec/)),trim(varname)//' in ncd_getvar_real_sp_4d')

  end subroutine ncd_getvar_real_sp_4d
!----------------------------------------------------------------------
  subroutine ncd_getvar_real_sp_5d(ncid, varname, rec, data)
  !
  ! DESCRIPTION
  ! read 3d real array
...
!**********************************************************************
  ...
  call check_ret( nf90_get_var(ncid%fh, vardesc%varid, data,  &
    start = (/1,1,1,1,1,rec/)),trim(varname)//' in ncd_getvar_real_sp_5d')

  end subroutine ncd_getvar_real_sp_5d
...
!-----------------------------------------------------------------------
  ...
  function ncd_putatt_int(ncid,varid,attrib,value)result(status)
    !
    ! !DESCRIPTION:
  ...
  end function ncd_putatt_int
!-----------------------------------------------------------------------

  function ncd_putatt_int_1d(ncid,varid,attrib,value)result(status)
  ...
  end function ncd_putatt_int_1d
!-----------------------------------------------------------------------

  function ncd_putatt_real(ncid,varid,attrib,value,xtype)result(status)
    !
    ! !DESCRIPTION:
    ! put real attributes
    !
  ...
  end function ncd_putatt_real
!------------------------------------------------------------------------------------------
  function ncd_putatt_char(ncid,varid,attrib,value)result(status)
    !
    ! !DESCRIPTION:
...
!------------------------------------------------------------------------------------------
  ...
  subroutine ncd_inqvid(ncid,name,varid,vardesc,readvar)
    ...
    character(len=*),parameter :: subname=trim(mod_filename)//'::ncd_inqvid' ! subroutine name
    !-----------------------------------------------------------------------
  ...
  subroutine ncd_io_char_var0_start_glob(vardesc, data, flag, ncid, start )
    ...
    character(len=*),parameter :: subname=trim(mod_filename)//'::ncd_io_char_var0_start_glob'
  ...
  subroutine ncd_io_1d_log_glob(varname, data, flag, ncid, readvar, nt, posNOTonfile)
    ...
    character(len=32) :: vname              ! variable error checking
    type(var_desc_t)  :: vardesc            ! local vardesc pointer
    character(len=*),parameter :: subname=trim(mod_filename)//'::ncd_io_1d_log_glob'
  ...
  subroutine ncd_io_0d_int_glob(varname, data, flag, ncid, readvar, nt, posNOTonfile)
    ...
    integer(i4) :: temp(1)
    logical :: lreadv
    character(len=*),parameter :: subname=trim(mod_filename)//'::ncd_io_0d_int_glob'
  ...
  subroutine ncd_io_1d_int_glob(varname, data, flag, ncid, readvar, nt, posNOTonfile)
    ...
    if (flag == 'read') then      
      if(present(nt))then
        ...
        call ncd_getvar_int_all_1d(ncid, varname, data)
      endif
      if(present(readvar))readvar=.true.
    elseif(flag=='write')then
      if(present(nt))then
        call ncd_putvar_int_1d(ncid, varname, nt, data)
      else
        call ncd_putvar_int_all_1d(ncid, varname, data)
      endif
    endif
...
!-------------------------------------------------------------------------------
  subroutine ncd_io_2d_int_glob(varname, data, flag, ncid, readvar, nt, posNOTonfile)
    ...
    type(var_desc_t)  :: vardesc            ! local vardesc pointer
    integer(i4) :: temp(1)
    character(len=*),parameter :: subname=trim(mod_filename)//'::ncd_io_2d_int_glob'

    if (flag == 'read') then
      if(present(nt))then
        call ncd_getvar_int_2d(ncid, varname, nt, data)
      else
        call ncd_getvar_int_all_2d(ncid, varname, data)
      endif
      if(present(readvar))readvar=.true.
    elseif(flag=='write')then
      if(present(nt))then
        call ncd_putvar_int_2d(ncid, varname, nt, data)
      else
        call ncd_putvar_int_all_2d(ncid, varname, data)
      endif
    endif
  ...
  subroutine ncd_io_3d_int_glob(varname, data, flag, ncid, readvar, nt, posNOTonfile)
    ...
    type(var_desc_t)  :: vardesc            ! local vardesc pointer
    integer(i4) :: temp(1)
    character(len=*),parameter :: subname=trim(mod_filename)//'::ncd_io_3d_int_glob'

    if (flag == 'read') then
      if(present(nt))then
        call ncd_getvar_int_3d(ncid, varname, nt, data)
      else
        call ncd_getvar_int_all_3d(ncid, varname, data)
      endif
      if(present(readvar))readvar=.true.
    elseif(flag=='write')then
      if(present(nt))then
        call ncd_putvar_int_3d(ncid, varname, nt, data)
      else
...
!------------------------------------------------------------------------------------------
  subroutine ncd_io_1d_double_glob(varname, data, flag, ncid, readvar, nt, posNOTonfile)
    ...
    type(var_desc_t)  :: vardesc            ! local vardesc pointer
    real(r8) :: temp(1)
    character(len=*),parameter :: subname=trim(mod_filename)//'::ncd_io_1d_double_glob'
...
!------------------------------------------------------------------------------------------
  subroutine ncd_io_2d_double_glob(varname, data, flag, ncid, readvar, nt, posNOTonfile)
    ...
    type(var_desc_t)  :: vardesc            ! local vardesc pointer
    real(r8) :: temp(1)
    character(len=*),parameter :: subname=trim(mod_filename)//'::ncd_io_2d_double_glob'
  ...
  subroutine ncd_io_3d_double_glob(varname, data, flag, ncid, readvar, nt, posNOTonfile)
    ...
    type(var_desc_t)  :: vardesc            ! local vardesc pointer
    real(r8) :: temp(1)
    character(len=*),parameter :: subname=trim(mod_filename)//'::ncd_io_3d_double_glob'
  ...
  !-----------------------------------------------------------------------
  subroutine ncd_io_4d_double_glob(varname, data, flag, ncid, readvar, nt, posNOTonfile)
    !
    ! !DESCRIPTION:
    ! netcdf I/O of global variable
    !
    ...
    type(var_desc_t)  :: vardesc            ! local vardesc pointer
    real(r8) :: temp(1)
    character(len=*),parameter :: subname=trim(mod_filename)//'::ncd_io_4d_double_glob'
  ...
  subroutine ncd_io_0d_text_glob(varname, data, flag, ncid, readvar, nt, posNOTonfile)
    ...
    character(len=1)  :: tmpString(128)     ! temp for manipulating output string
    type(var_desc_t)  :: vardesc            ! local vardesc pointer
    character(len=*),parameter :: subname=trim(mod_filename)//'::ncd_io_0d_text_glob'

    print*,'subname ',subname,' ',trim(varname)
  ...
  subroutine ncd_io_1d_text_glob(varname, data, flag, ncid, readvar, nt, posNOTonfile)
    ...
    character(len=1)  :: tmpString(128)     ! temp for manipulating output string
    type(var_desc_t)  :: vardesc            ! local vardesc pointer
    character(len=*),parameter :: subname=trim(mod_filename)//'::ncd_io_1d_text_glob'
    logical :: lreadv
    ...
    if(flag=='write')then
      if(present(nt))then
        ...
        call check_ret( nf90_put_var(ncid%fh, vardesc%varid, data),trim(subname))
      endif
    else
      if(present(readvar))readvar=lreadv
  ...
  subroutine ncd_io_2d_text_glob(varname, data, flag, ncid, readvar, nt, posNOTonfile)
    ...
    character(len=1)  :: tmpString(128)     ! temp for manipulating output string
    type(var_desc_t)  :: vardesc            ! local vardesc pointer
    character(len=*),parameter :: subname=trim(mod_filename)//'::ncd_io_2d_text_glob'

    print*,'subname ',subname,' ',varname
...
!----------------------------------------------------------------------
  ...
  subroutine ncd_io_3d_double(varname, data, dim1name, flag, ncid, nt, readvar)
    ...
    type(iodesc_plus_type) , pointer :: iodesc_plus
    type(var_desc_t)                 :: vardesc
    character(len=*),parameter :: subname=trim(mod_filename)//'::ncd_io_3d_double' ! subroutine name
    ...
    !-----------------------------------------------------------------------
  subroutine ncd_io_4d_double(varname, data, dim1name, flag, ncid, nt, readvar)
    !
    ! !DESCRIPTION:
    ! Netcdf i/o of 3d
    !
    ...
    type(iodesc_plus_type) , pointer :: iodesc_plus
    type(var_desc_t)                 :: vardesc
    character(len=*),parameter :: subname=trim(mod_filename)//'::ncd_io_4d_double' ! subroutine name
  ...
  subroutine ncd_io_5d_double(varname, data, dim1name, flag, ncid, nt, readvar)
    ...
    type(iodesc_plus_type) , pointer :: iodesc_plus
    type(var_desc_t)                 :: vardesc
    character(len=*),parameter :: subname=trim(mod_filename)//'::ncd_io_5d_double' ! subroutine name
  ...
  ncd_inq_varid = nf90_inq_varid (ncid%fh, varname, vardesc%varid)
