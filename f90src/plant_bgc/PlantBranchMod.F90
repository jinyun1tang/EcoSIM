...
!
    ...
    !     LeafAreaNode_brch,CanopyLeafArea_lpft=leaf node surface area in canopy layer
    ...
    call GrainFilling(I,NB,NZ,Growth_brch,Growth_brch(ielmc,ibrch_stalk))
!
...
!------------------------------------------------------------------------------------------
  ...
  real(r8), intent(in) :: NonstC4Groth_brch
  real(r8), intent(in) ::  DMLFB,DMSHB,CNLFB,CPLFB,CNSHB,CPSHB,ZPLFD,CNPG
  ...
  IF(iPlantCalendar_brch(ipltcal_InitFloral,NB,NZ).NE.0)THEN
    IF(StalkRsrvElms_brch(ielmc,NB,NZ).LT.XFRX*StalkBiomassC_brch(NB,NZ))THEN
      D1020: DO N=1,NumOfPlantMorphUnits
        IF(N.NE.4)THEN
          PART(ibrch_resrv)=PART(ibrch_resrv)+0.10*PART(N)
...
!
    ELSEIF(StalkRsrvElms_brch(ielmc,NB,NZ).GT.1.0_r8*StalkBiomassC_brch(NB,NZ))THEN
...
!  if(NZ==1)THEN
!    write(101,'(I3,8(X,F13.6))')NB,I+J/24.,PART
!  ELSE
!    write(102,'(I3,8(X,F13.6))')NB,I+J/24.,PART
!  ENDIF
  end associate
...
!
      ...
      D6310: DO M=1,jsken
        DO NE=1,NumPlantChemElms
          LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ)=LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ)&
            +ElmAllocmat4Litr(NE,icwood,M,NZ)*Frac2Remobl*(LeafElmntNode_brch(NE,K,NB,NZ) &
          ...
          LitrfalStrutElms_pvr(NE,M,k_fine_litr,0,NZ)=LitrfalStrutElms_pvr(NE,M,k_fine_litr,0,NZ) &
            +ElmAllocmat4Litr(NE,ifoliar,M,NZ)*Frac2Remobl*(LeafElmntNode_brch(NE,K,NB,NZ) &
      ...
      IF(K.NE.0)THEN
        LitrfalStrutElms_pvr(ielmc,icarbhyro,k_fine_litr,0,NZ)=LitrfalStrutElms_pvr(ielmc,icarbhyro,k_fine_litr,0,NZ) &
          +Frac2Remobl*(CPOOL3_node(K,NB,NZ)+CPOOL4_node(K,NB,NZ))
        CPOOL3_node(K,NB,NZ)=CPOOL3_node(K,NB,NZ)-Frac2Remobl*CPOOL3_node(K,NB,NZ)
        CPOOL4_node(K,NB,NZ)=CPOOL4_node(K,NB,NZ)-Frac2Remobl*CPOOL4_node(K,NB,NZ)
      ENDIF
!
...
!
      ...
      DO NE=1,NumPlantChemElms
        ...
        LeafElmntNode_brch(NE,K,NB,NZ)=LeafElmntNode_brch(NE,K,NB,NZ)-Frac2Remobl*LeafElmntNode_brch(NE,K,NB,NZ)
      ENDDO
      LeafProteinCNode_brch(K,NB,NZ)=AZMAX1(LeafProteinCNode_brch(K,NB,NZ) &
        -Frac2Remobl*AMAX1(LeafElmntNode_brch(ielmn,K,NB,NZ)*rCNNonstructRemob_pft(NZ) &
...
!
    ...
    !
      ...
      !
!        IF(FracSensResp4Leaf.LE.ZEROP(NZ))GO TO 564
      !
...
!     EXIT LOOP IF REMOBILIZATION REQUIREMENT HAS BEEN MET
!
      lgoto565=(SensResp2Use.LE.ZEROP(NZ))
!
    ...
    ELSE      
      D6325: DO M=1,jsken
        DO NE=1,NumPlantChemElms
          LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ)=LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ) &
            +ElmAllocmat4Litr(NE,icwood,M,NZ)*PetioleElmntNode_brch(NE,K,NB,NZ)*FracShootLeafElmAlloc2Litr(NE,k_woody_litr)
          LitrfalStrutElms_pvr(NE,M,k_fine_litr,0,NZ)=LitrfalStrutElms_pvr(NE,M,k_fine_litr,0,NZ) &
            +ElmAllocmat4Litr(NE,inonfoliar,M,NZ)*PetioleElmntNode_brch(NE,K,NB,NZ)*FracShootLeafElmAlloc2Litr(NE,k_fine_litr)
        ENDDO    
      ENDDO D6325
      DO NE=1,NumPlantChemElms
...
!
          ...
          D7310: DO M=1,jsken
            DO NE=1,NumPlantChemElms
              LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ)=LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ)&
                +ElmAllocmat4Litr(NE,istalk,M,NZ)*FSNCK &
                *(InternodeStrutElms_brch(NE,K,NB,NZ)-RCEK(NE))*FracRootStalkElmAlloc2Litr(NE,k_woody_litr)

              LitrfalStrutElms_pvr(NE,M,k_fine_litr,0,NZ)=LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ)&
                +ElmAllocmat4Litr(NE,istalk,M,NZ)*FSNCK &
                *(InternodeStrutElms_brch(NE,K,NB,NZ)-RCEK(NE))*FracRootStalkElmAlloc2Litr(NE,k_fine_litr)
            ENDDO    
          ENDDO D7310
...
!
      ...
      !
      !     FSNCK=fraction of lowest internode to be remobilized
      !     LiveInterNodeHight_brch,InternodeHeightDying_brch=living,senescing internode length
      ...
      !
          ...
          ENDDO
          LiveInterNodeHight_brch(K,NB,NZ)=LiveInterNodeHight_brch(K,NB,NZ)-FSNCK*InternodeHeightDying_brch(K,NB,NZ)
    ...
    !
        ELSE
          ...
          D7315: DO M=1,jsken
            DO NE=1,NumPlantChemElms
              LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ)=LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ)&
                +ElmAllocmat4Litr(NE,istalk,M,NZ)*InternodeStrutElms_brch(NE,K,NB,NZ)*FracRootStalkElmAlloc2Litr(NE,k_woody_litr)

              LitrfalStrutElms_pvr(NE,M,k_fine_litr,0,NZ)=LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ) &
                +ElmAllocmat4Litr(NE,istalk,M,NZ)*InternodeStrutElms_brch(NE,K,NB,NZ)*FracRootStalkElmAlloc2Litr(NE,k_fine_litr)
            ENDDO    
          ENDDO D7315
          DO NE=1,NumPlantChemElms
          ...
          ENDDO
          LiveInterNodeHight_brch(K,NB,NZ)=LiveInterNodeHight_brch(K,NB,NZ)-InternodeHeightDying_brch(K,NB,NZ)
...
!
    ...
    !
        D8310: DO M=1,jsken
          DO NE=1,NumPlantChemElms
            LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ)=LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ)&
              +ElmAllocmat4Litr(NE,istalk,M,NZ)*FSNCR*(SenecStalkStrutElms_brch(NE,NB,NZ)-RCEK(NE))*FracRootStalkElmAlloc2Litr(NE,k_woody_litr)

            LitrfalStrutElms_pvr(NE,M,k_fine_litr,0,NZ)=LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ)&
              +ElmAllocmat4Litr(NE,istalk,M,NZ)*FSNCR*(SenecStalkStrutElms_brch(NE,NB,NZ)-RCEK(NE))*FracRootStalkElmAlloc2Litr(NE,k_fine_litr)
          ENDDO    
        ENDDO D8310
...
!
      ...
      LeafLength=AZMAX1(SQRT(rLen2WidthLeaf(NZ)*AZMAX1(LeafAreaNode_brch(K,NB,NZ))/(PlantPopulation_pft(NZ)*FracGroth2Node_pft(NZ))))
...
!------------------------------------------------------------------------------------------
  ...
  subroutine GrainFilling(I,NB,NZ,Growth_brch,GROSTKC)
  implicit none
...
!   SeedTempSens_pft=sensitivity to TCelciusCanopy_pft> HTC,TCelciusCanopy_pft< TCChill4Seed_pftfrom startq.f (seeds oC-1)
  ...
  IF(iPlantCalendar_brch(ipltcal_Anthesis,NB,NZ).NE.0.AND.iPlantCalendar_brch(ipltcal_SetSeedMass,NB,NZ).EQ.0)THEN
    ...
    IF(TCelciusCanopy_pft(NZ).LT.TCChill4Seed_pft(NZ))THEN
      IF(iPlantCalendar_brch(ipltcal_BeginSeedFill,NB,NZ).EQ.0)THEN
        FGRNX=SeedTempSens_pft(NZ)*(TCChill4Seed_pft(NZ)-TCelciusCanopy_pft(NZ))
      ELSEIF(iPlantCalendar_brch(ipltcal_SetSeedNumber,NB,NZ).EQ.0)THEN
        FGRNX=SeedTempSens_pft(NZ)*(TCChill4Seed_pft(NZ)-TCelciusCanopy_pft(NZ))
      ELSE
        FGRNX=0._r8
      ENDIF
    ELSEIF(TCelciusCanopy_pft(NZ).GT.HighTempLimitSeed_pft(NZ))THEN
      IF(iPlantCalendar_brch(ipltcal_BeginSeedFill,NB,NZ).EQ.0)THEN
        FGRNX=SeedTempSens_pft(NZ)*(TCelciusCanopy_pft(NZ)-HighTempLimitSeed_pft(NZ))
      ELSEIF(iPlantCalendar_brch(ipltcal_SetSeedNumber,NB,NZ).EQ.0)THEN
        FGRNX=SeedTempSens_pft(NZ)*(TCelciusCanopy_pft(NZ)-HighTempLimitSeed_pft(NZ))
      ELSE
        FGRNX=0._r8
...
!------------------------------------------------------------------------------------------
  ...
  subroutine ResetBranchPhenology(I,NB,NZ)
            ...
            D5330: DO M=1,jsken
              DO NE=1,NumPlantChemElms
                LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ)=LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ) &
                  ...
                  +ElmAllocmat4Litr(NE,icwood,M,NZ)*PetoleStrutElms_brch(NE,NB,NZ)*FracShootLeafElmAlloc2Litr(NE,k_woody_litr)
...
!
  IF(Prep4Literfall_brch(NB,NZ).EQ.itrue)THEN
    ...
    D6330: DO M=1,jsken
      DO NE=1,NumPlantChemElms
        LitrfalStrutElms_pvr(NE,M,k_fine_litr,0,NZ)=LitrfalStrutElms_pvr(NE,M,k_fine_litr,0,NZ)+ &
          FSNR*ElmAllocmat4Litr(NE,inonfoliar,M,NZ)*(HuskStrutElms_brch(NE,NB,NZ)+EarStrutElms_brch(NE,NB,NZ))
        IF(iPlantPhenolPattern_pft(NZ).EQ.iplt_annual.AND.iPlantPhenolType_pft(NZ).NE.0)THEN
          SeasonalNonstElms_pft(NE,NZ)=SeasonalNonstElms_pft(NE,NZ)+ &
            FSNR*ElmAllocmat4Litr(NE,inonfoliar,M,NZ)*GrainStrutElms_brch(NE,NB,NZ)
!          print*,'plantbranch2375NonStrutElms_pft(NE,NZ)',NZ,NE,SeasonalNonstElms_pft(NE,NZ)  
        ELSE
          LitrfalStrutElms_pvr(NE,M,k_fine_litr,0,NZ)=LitrfalStrutElms_pvr(NE,M,k_fine_litr,0,NZ)+ &
            FSNR*ElmAllocmat4Litr(NE,inonfoliar,M,NZ)*GrainStrutElms_brch(NE,NB,NZ)
        ENDIF
...
!
    IF((iPlantTurnoverPattern_pft(NZ).EQ.0 .OR. (.not.is_plant_treelike(iPlantRootProfile_pft(NZ)))) &
      ...
      D6335: DO M=1,jsken
        DO NE=1,NumPlantChemElms
          LitrfalStrutElms_pvr(NE,M,k_fine_litr,0,NZ)=LitrfalStrutElms_pvr(NE,M,k_fine_litr,0,NZ)&
            +FSNR*ElmAllocmat4Litr(NE,istalk,M,NZ)*StalkStrutElms_brch(NE,NB,NZ)
        ENDDO
      ENDDO D6335
      DO NE=1,NumPlantChemElms  
  ...
  ! main branch leaf out
    IF(NB.EQ.MainBranchNum_pft(NZ))THEN
      ...
      IF(Hours2LeafOut_brch(NB,NZ).LE.HourReq2InitSStor4LeafOut(iPlantPhenolPattern_pft(NZ)).OR.(plantChk))THEN
        ...
        IF(SeasonalNonstElms_pft(ielmc,NZ).GT.ZEROP(NZ))THEN
...
!         if root condition met
    ...
    !
      ...
      IF(SeasonalNonstElms_pft(ielmc,NZ).GT.ZEROP(NZ))THEN
        IF(iPlantPhenolPattern_pft(NZ).NE.iplt_annual)THEN
          CPOOLT=AMAX1(ZEROP(NZ),SeasonalNonstElms_pft(ielmc,NZ)+TotalRootNonstElms(ielmc))
          DO NE=2,NumPlantChemElms
    ...
    IF(iPlantPhenolPattern_pft(NZ).EQ.iplt_annual.AND.&
      ...
      D2050: DO L=NU,MaxSoiL4Root(NZ)
        IF(VLSoilPoreMicP_vr(L).GT.ZEROS2)THEN
          WTRTRX=AMAX1(ZEROP(NZ),RootMycoActiveBiomC_pvr(ipltroot,L,NZ)*FracRootElmAlloc2Litr(ielmc,k_woody_litr))
...
!
  ...
  RCO2NonstC_brch=RCO2CM*RAutoRootO2Limter_pvr(ipltroot,NGTopRootLayer_pft(NZ),NZ)
!
! MAINTENANCE RESPIRATION FROM TEMPERATURE, PLANT STRUCTURAL N
!
! RCO2Maint_brch=maintenance respiration
...
!------------------------------------------------------------------------------------------
  ...
  subroutine GrowLeavesOnBranch(NZ,NB,MinNodeNum,GrowthLeaf,EtoliationCoeff,WFNS,ALLOCL)
  implicit none
  integer , intent(in) :: NB,NZ
...
!         LeafAreaLive_brch,LeafAreaNode_brch=branch,node leaf area
!
      SpecAreaLeafGrowth=EtoliationCoeff*SLA1(NZ)*(AMAX1(ZEROL(NZ) &
...
!
      IF(LeafElmntNode_brch(ielmc,K,NB,NZ).GT.0.0_r8)THEN
        SSL=EtoliationCoeff*PetoLen2Mass_pft(NZ)*(AMAX1(ZEROL(NZ) &
...
!
    D510: DO KK=MNNOD,MXNOD
      ...
      IF(K1.NE.0)THEN
        LiveInterNodeHight_brch(K1,NB,NZ)=InternodeHeightDying_brch(K1,NB,NZ) &
          +LiveInterNodeHight_brch(K2,NB,NZ)
      ELSE
        LiveInterNodeHight_brch(K1,NB,NZ)=InternodeHeightDying_brch(K1,NB,NZ)
      ENDIF
    ENDDO D510
...
!
      DO NE=1,NumPlantChemElms
        D6300: DO M=1,jsken
          dRemoblE=FracRemobAsLeaf*(LeafChemElmRemob_brch(NE,NB,NZ)-LeafElmntRemobFlx_brch(NE,NB,NZ)) 
          LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ)=LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ) &
...
!
      ...
      LeafProteinCNode_brch(K,NB,NZ)=AZMAX1(LeafProteinCNode_brch(K,NB,NZ) &
        -FracRemobAsLeaf*AMAX1(LeafChemElmRemob_brch(ielmn,NB,NZ)*rCNNonstructRemob_pft(NZ) &
...
!
      ...
      D6305: DO M=1,jsken
        DO NE=1,NumPlantChemElms
          dRemoblE=FSNCS*(PetioleChemElmRemob_brch(NE,NB,NZ)-PetioleChemElmRemobFlx_brch(NE,NB,NZ))
          LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ)=LitrfalStrutElms_pvr(NE,M,k_woody_litr,0,NZ) &
