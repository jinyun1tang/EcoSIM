...
!
          ...
          ENDIF
          DO NGL=1,JG
            OMC2(NGL,N,K)=AMAX1(0.0,AMIN1(OMA(NGL,N,K)*FL(2),OMC(2,NGL,N,K)))
...
!     nitrous acid concn CHNO2
  ...
  RCNO2=AMAX1(0.0,AMIN1(ZNO2S*FNO2,VMXC4S))
  RCNOB=AMAX1(0.0,AMIN1(ZNO2B*FNB2,VMXC4B))
  RCN2O=0.10*RCNO2
  RCN2B=0.10*RCNOB
  RCNO3=0.80*RCNO2
  RCN3B=0.80*RCNOB
...
!
  IF(VOLWM(NPH).GT.ZEROS2 &
    .AND.FOSRH(K).GT.ZERO)THEN
    IF(litrm)THEN
      AECX=0.5E+03_r8
    ELSE
      AECX=AEC
...
!
    DO 785 M=1,jsken
      IF(OSC(M,K).GT.ZEROS)THEN
        CNS(M,K)=AMAX1(0.0,OSN(M,K)/OSC(M,K))
        CPS(M,K)=AMAX1(0.0,OSP(M,K)/OSC(M,K))
        RDOSC(M,K)=AMAX1(0.0,AMIN1(0.5*OSA(M,K) &
          ,SPOSC(M,K)*ROQCK(K)*DFNS*OQCI*TFNX*OSA(M,K)/OSRH(K)))
        RDOSN(M,K)=AMAX1(0.0,AMIN1(OSN(M,K),CNS(M,K)*RDOSC(M,K)))/FCNK(K)
        RDOSP(M,K)=AMAX1(0.0,AMIN1(OSP(M,K),CPS(M,K)*RDOSC(M,K)))/FCPK(K)
  ...
  IF(OSRH(K).GT.ZEROS)THEN
    DO 775 M=1,2
      IF(ORC(M,K).GT.ZEROS)THEN
        CNR=AMAX1(0.0,ORN(M,K)/ORC(M,K))
        CPR=AMAX1(0.0,ORP(M,K)/ORC(M,K))
        RDORC(M,K)=AMAX1(0.0,AMIN1(ORC(M,K) &
          ,SPORC(M)*ROQCK(K)*DFNS*OQCI*TFNX*ORC(M,K)/OSRH(K)))
    !    3*AMIN1(FCNK(K),FCPK(K))
        RDORN(M,K)=AMAX1(0.0,AMIN1(ORN(M,K),CNR*RDORC(M,K)))/FCNK(K)
        RDORP(M,K)=AMAX1(0.0,AMIN1(ORP(M,K),CPR*RDORC(M,K)))/FCPK(K)
      ELSE
        RDORC(M,K)=0.0_r8
        RDORN(M,K)=0.0_r8
        RDORP(M,K)=0.0_r8
...
!
  IF(OSRH(K).GT.ZEROS)THEN
    IF(OHC(K).GT.ZEROS)THEN
      CNH(K)=AMAX1(0.0,OHN(K)/OHC(K))
      CPH(K)=AMAX1(0.0,OHP(K)/OHC(K))
      RDOHC(K)=AMAX1(0.0,AMIN1(OHC(K) &
        ,SPOHC*ROQCK(K)*DFNS*OQCI*TFNX*OHC(K)/OSRH(K)))
!    3*AMIN1(FCNK(K),FCPK(K))
      RDOHN(K)=AMAX1(0.0,AMIN1(OHN(K),CNH(K)*RDOHC(K)))/FCNK(K)
      RDOHP(K)=AMAX1(0.0,AMIN1(OHP(K),CPH(K)*RDOHC(K)))/FCPK(K)
      RDOHA(K)=AMAX1(0.0,AMIN1(OHA(K) &
...
!
  ...
  RGOCX=AMAX1(0.0,OQC(K)*FOQC*EO2Q)
  RGOAX=AMAX1(0.0,OQA(K)*FOQA*EO2A)
...
!
  ...
  OQCZ3=AMAX1(0.0,OQC(K)*FOQC-RGOCP*WFN(NGL,N,K))
  OQCD3=OQCZ3/ECN3
  OQCD3S=OQCD3*FNO3S
  OQCD3B=OQCD3*FNO3B
  ZNO3SX=ZNO3S*FNO3
...
!
  ...
  IF(RINHP.GT.0.0)THEN
    ...
    RINH4(NGL,N,K)=AMIN1(FNH4X*AMAX1(0.0,(ZNH4S-ZNH4M)) &
      ,RINHO(NGL,N,K))
    RINB4(NGL,N,K)=AMIN1(FNB4X*AMAX1(0.0,(ZNH4B-ZNHBM)) &
      ,RINHB(NGL,N,K))
  ELSE
...
!
  ...
  IF(RIPOP.GT.0.0)THEN
    ...
    RIPO4(NGL,N,K)=AMIN1(FPO4X*AMAX1(0.0,(H2PO4-H2POM)) &
      ,RIPOO(NGL,N,K))
    RIPOB(NGL,N,K)=AMIN1(FPOBX*AMAX1(0.0,(H2POB-H2PBM)) &
      ,RIPBO(NGL,N,K))
  ELSE
...
!
  ...
  IF(RIP1P.GT.0.0)THEN
    CH1PX=AMAX1(0.0,CH1P4-HPMN)
    CH1PY=AMAX1(0.0,CH1P4B-HPMN)
    ...
    RIP14(NGL,N,K)=AMIN1(FP14X*AMAX1(0.0,(H1PO4-H1POM)) &
      ,RIPO1(NGL,N,K))
    RIP1B(NGL,N,K)=AMIN1(FP1BX*AMAX1(0.0,(H1POB-H1PBM)) &
      ,RIPB1(NGL,N,K))
  ELSE
...
!
  IF(litrm)THEN
    ...
    IF(RINHPR.GT.0.0)THEN
      CNH4X=AMAX1(0.0,CNH4SU-Z4MN)
      CNH4Y=AMAX1(0.0,CNH4BU-Z4MN)
...
!
    ...
    IF(RINOPR.GT.0.0)THEN
      CNO3X=AMAX1(0.0,CNO3SU-ZOMN)
      CNO3Y=AMAX1(0.0,CNO3BU-ZOMN)
...
!
    ...
    IF(RIPOPR.GT.0.0)THEN
      CH2PX=AMAX1(0.0,CH2P4U-HPMN)
      CH2PY=AMAX1(0.0,CH2P4BU-HPMN)
...
!
    ...
    IF(RIP1PR.GT.0.0)THEN
      CH1PX=AMAX1(0.0,CH1P4U-HPMN)
      CH1PY=AMAX1(0.0,CH1P4BU-HPMN)
...
!
  IF(N.EQ.n_aero_n2fixer.OR.N.EQ.n_anero_n2fixer)THEN
    RGN2P=AMAX1(0.0,OMC(3,NGL,N,K)*CNOMC(3,NGL,N,K) &
...
!
  ...
  CGOMN(NGL,N,K)=AMAX1(0.0,AMIN1(OQN(K)*FOMK(NGL,N,K) &
    ,CGOXC*CNQ(K)/FCN(NGL,N,K)))
  CGOMP(NGL,N,K)=AMAX1(0.0,AMIN1(OQP(K)*FOMK(NGL,N,K) &
...
! M=1:labile, 2, resistant
  ...
  DO 745 M=1,2
    ...
    IF(OMC(3,NGL,N,K).GT.ZEROS)THEN
      CGONS(M,NGL,N,K)=AMIN1(FL(M)*AMAX1(0.0,OMN(3,NGL,N,K)) &
        ,CGOMS(M,NGL,N,K)*OMN(3,NGL,N,K)/OMC(3,NGL,N,K))
      CGOPS(M,NGL,N,K)=AMIN1(FL(M)*AMAX1(0.0,OMP(3,NGL,N,K)) &
...
!
    ...
    RXOMC(M,NGL,N,K)=AMAX1(0.0,OMC(M,NGL,N,K)*SPOMX)
    RXOMN(M,NGL,N,K)=AMAX1(0.0,OMN(M,NGL,N,K)*SPOMX)
    RXOMP(M,NGL,N,K)=AMAX1(0.0,OMP(M,NGL,N,K)*SPOMX)
...
!     RCOMC,RCOMN,RCOMP=transfer of microbial C,N,P litterfall to residue
!
    RHOMC(M,NGL,N,K)=AMAX1(0.0,RDOMC(M,NGL,N,K)*EHUM)
    RHOMN(M,NGL,N,K)=AMAX1(0.0,RDOMN(M,NGL,N,K)*EHUM)
    RHOMP(M,NGL,N,K)=AMAX1(0.0,RDOMP(M,NGL,N,K)*EHUM)
!
...
!     RCMMC,RCMMN,RCMMC=transfer of senesence litterfall C,N,P to residue
!
      RHMMC(M,NGL,N,K)=AMAX1(0.0,RDMMC(M,NGL,N,K)*EHUM)
      RHMMN(M,NGL,N,K)=AMAX1(0.0,RDMMN(M,NGL,N,K)*EHUM)
      RHMMP(M,NGL,N,K)=AMAX1(0.0,RDMMP(M,NGL,N,K)*EHUM)
