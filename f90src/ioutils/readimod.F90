...
!  the extra soil layer below root zone cannot be greater than what is allowed
    ...
    call ncd_getvar(grid_nfid, 'CORGC',ntp,CORGC_vr(1:JZ,NV1,NH1))
    call ncd_getvar(grid_nfid, 'CORGR',ntp,COMLitrC_vr(1:JZ,NV1,NH1))
    call ncd_getvar(grid_nfid, 'CORGN',ntp,CORGN_vr(1:JZ,NV1,NH1))
    call ncd_getvar(grid_nfid, 'CORGP',ntp,CORGP_vr(1:JZ,NV1,NH1))
...
!
        IF (NX/=NH1 .OR. NY/=NV1) THEN
          DO L=NU(NY,NX),NM(NY,NX)
            ...
            CORGC_vr(L,NY,NX)=CORGC_vr(L,NV1,NH1)
            COMLitrC_vr(L,NY,NX)=COMLitrC_vr(L,NV1,NH1)
            CORGN_vr(L,NY,NX)=CORGN_vr(L,NV1,NH1)
            CORGP_vr(L,NY,NX)=CORGP_vr(L,NV1,NH1)
        ...
        IF(NU(NY,NX).GT.1)THEN
          DO  L=NU(NY,NX)-1,0,-1
            ...
            IF(L.GT.0)THEN
              ...
              CORGC_vr(L,NY,NX)=1.0_r8*CORGC_vr(L+1,NY,NX)
              COMLitrC_vr(L,NY,NX)=1.0_r8*COMLitrC_vr(L+1,NY,NX)
              CORGN_vr(L,NY,NX)=1.0_r8*CORGN_vr(L+1,NY,NX)
              CORGP_vr(L,NY,NX)=1.0_r8*CORGP_vr(L+1,NY,NX)
...
!   SoiBulkDensityt0: initial bulk density
  ...
  !
          ...
          CORGC_vr(L,NY,NX)=CORGC_vr(L,NY,NX)*1.0E+03_r8   !convert from Kg to g C
          COMLitrC_vr(L,NY,NX)=COMLitrC_vr(L,NY,NX)*1.0E+03_r8   !convert from Kg to g C
          CORGCI(L,NY,NX)=CORGC_vr(L,NY,NX)
          SoilFracAsMacPt0(L,NY,NX)=SoilFracAsMacP(L,NY,NX)
  !
          CSAND(L,NY,NX)=CSAND(L,NY,NX)*1.0E-03_r8*AZMAX1((1.0_r8-CORGC_vr(L,NY,NX)/orgcden))
          CSILT(L,NY,NX)=CSILT(L,NY,NX)*1.0E-03_r8*AZMAX1((1.0_r8-CORGC_vr(L,NY,NX)/orgcden))
          CCLAY(L,NY,NX)=CCLAY(L,NY,NX)*1.0E-03_r8*AZMAX1((1.0_r8-CORGC_vr(L,NY,NX)/orgcden))
  ...
  !
          IF(CORGN_vr(L,NY,NX).LT.0.0_r8)THEN
  !  default ORGN parameterization
            CORGN_vr(L,NY,NX)=AMIN1(0.125_r8*CORGC_vr(L,NY,NX),8.9E+02_r8*(CORGC_vr(L,NY,NX)/1.0E+04_r8)**0.80_r8)
          ENDIF
          IF(CORGP_vr(L,NY,NX).LT.0.0_r8)THEN
            CORGP_vr(L,NY,NX)=AMIN1(0.0125_r8*CORGC_vr(L,NY,NX),1.2E+02_r8*(CORGC_vr(L,NY,NX)/1.0E+04_r8)**0.52_r8)
          ENDIF
          IF(CEC(L,NY,NX).LT.0.0_r8)THEN
            !estimate from input data
            CEC(L,NY,NX)=10.0_r8*(200.0_r8*2.0_r8*CORGC_vr(L,NY,NX)/1.0E+06_r8 &
