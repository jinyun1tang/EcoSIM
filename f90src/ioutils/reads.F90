...
!23   READ(6,'(F8.3,4X,A8,I8,50E16.7E3)',END=27)DOY,CDATE,J
!    2,(OUT(L),L=1,33)
!     IF(J.EQ.24)THEN
!     I=INT(DOY)
!     ELSE
...
!
      IF(TTYPE.EQ.'D')THEN
        call readdayweather(GO110,GO60)
        IF(GO60)GO TO 60
        IF(GO110)GO TO 110
!
...
200   CONTINUE
      ...
      subroutine readdayweather(go110,go60)
...
!
      ...
      DWPT(1,I)=0.61*EXP(5360.0*(3.661E-03-1.0 &
      /(273.15+(TMPN(I)+TMPX(I))/2)))*DAT(K)
      DWPT(2,I)=0.61*EXP(5360.0*(3.661E-03-1.0 &
      /(273.15+TMPN(I))))
      ELSEIF(TYP(K).EQ.'R')THEN
      DAT(K)=AMAX1(0.0,AMIN1(100.0,DAT(K)))
      DWPT(1,I)=0.61*EXP(5360.0*(3.661E-03-1.0 &
      /(273.15+(TMPN(I)+TMPX(I))/2)))*DAT(K)*0.01
      DWPT(2,I)=0.61*EXP(5360.0*(3.661E-03-1.0 &
      /(273.15+TMPN(I))))
...
!------------------------------------------------------------------------------------------
      ...
      subroutine readhourweather(go60)
