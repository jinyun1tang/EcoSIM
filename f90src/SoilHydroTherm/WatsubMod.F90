...
!
    ...
    IF(VOLTX.GT.ZEROS2(NY,NX))THEN
      THETWX(L,NY,NX)=AMAX1(0.0_r8,(VOLW1(L,NY,NX)+VOLWH1(L,NY,NX))/VOLTX)
      THETIX(L,NY,NX)=AMAX1(0.0_r8,(VOLI1(L,NY,NX)+VOLIH1(L,NY,NX))/VOLTX)
      THETPX(L,NY,NX)=AMAX1(0.0_r8,(VOLP1(L,NY,NX)+VOLPH1(L,NY,NX))/VOLTX)
    ELSE
    ...
    IF(VOLA1(L,NY,NX)+VOLAH1(L,NY,NX).GT.ZEROS2(NY,NX))THEN
      THETPY(L,NY,NX)=AMAX1(0.0_r8,(VOLP1(L,NY,NX)+VOLPH1(L,NY,NX)) &
...
!
  ...
  VOLW1(0,NY,NX)=AMAX1(0.0_r8,VOLW(0,NY,NX))
  VOLI1(0,NY,NX)=AMAX1(0.0_r8,VOLI(0,NY,NX))
  ...
  IF(VOLR(NY,NX).GT.ZEROS2(NY,NX))THEN
    ...
    THETPX(0,NY,NX)=AMAX1(0.0_r8,VOLP1(0,NY,NX)/VOLR(NY,NX)) &
      *AMAX1(0.0_r8,(1.0-XVOLT(NY,NX)/VOLWD(NY,NX)))
  ELSE
...
! begin_execution
  DO 4000 NN=1,NPR
    ...
    IF(HFLWC.GE.0.0)THEN
      HFLWSRX=AMAX1(0.0_r8,AMIN1(HFLWX,HFLWC))
    ELSE
...
!
    IF(VOLPM(M,0,NY,NX).GT.ZEROS(NY,NX) &
      ...
      IF(FLVC.GE.0.0)THEN
        FLVR1X=AMAX1(0.0_r8,AMIN1(FLVC,FLVX,VOLW0M(L,NY,NX)*XNPB))
...
!
    ...
    IF(HFLWC.GE.0.0)THEN
      HFLWR1X=AMAX1(0.0_r8,AMIN1(HFLWX,HFLWC))
    ELSE
    ...
    IF(VHCPWMM(L,NY,NX).GT.VHCPWX(NY,NX))THEN
      ...
      IF(L.LT.JS.AND.VHCPWMM(L2,NY,NX).GT.VHCPWX(NY,NX))THEN
        ...
        IF(VOLP01.GT.ZEROS2(NY,NX).AND.VOLP02.GT.ZEROS2(NY,NX))THEN
          ...
          IF(FLVC.GE.0.0)THEN
            FLVSS=AMAX1(0.0_r8,AMIN1(FLVC,FLVX,VOLW0M(L,NY,NX)*XNPX))
        ...
        IF(HFLWC.GE.0.0)THEN
          HFLWSS=AMAX1(0.0_r8,AMIN1(HFLWX,HFLWC))
        ELSE
...
!      endif
      ...
      IF(THETPM(M,0,NY,NX).GT.THETX.AND.THETPM(M,NUM(NY,NX),NY,NX).GT.THETX)THEN
        ...
        IF(FLVC.GE.0.0)THEN
          !from litter to soil
          FLV2=AMAX1(0.0_r8,AMIN1(FLVC,FLVX))
      ...
      ! endif
      IF(HFLWC.GE.0.0)THEN
        HFLCR2=AMAX1(0.0_r8,AMIN1(HFLWX,HFLWC))
      ELSE
...
!
  ...
  EVAPG(NY,NX)=AMAX1(PARE*(VPQ(NY,NX)-VP1) &
      ,-AMAX1(0.0_r8,VOLW2(NUM(NY,NX),NY,NX)*XNPX))
...
!
  IF(BKDS(NUM(NY,NX),NY,NX).GT.ZERO)THEN
    FLQRS=AMAX1(0.0_r8,FLQ1(NY,NX)-VOLP1(NUM(NY,NX),NY,NX))
    FLQRH=AMAX1(0.0_r8,FLH1(NY,NX)-VOLPH1(NUM(NY,NX),NY,NX))
...
!
  ...
  IF(VOLAT0.GT.ZEROS2(NY,NX).AND.VOLPM(M,0,NY,NX).GT.ZEROS2(NY,NX))THEN
    THETWA=AMAX1(0.0_r8,AMIN1(1.0_r8,VOLW1(0,NY,NX)/VOLAT0))
...
!
  IF(PRECD(NY,NX).GT.ZERO)THEN
    ENGYD=AMAX1(0.0_r8,8.95_r8+8.44_r8*LOG(PRECM(NY,NX)))
  ELSE
    ENGYD=0.0_r8
  ENDIF
  IF(PRECB(NY,NX).GT.ZERO)THEN
    ENGYB=AMAX1(0.0_r8,15.8_r8*SQRT(AMIN1(2.5_r8,ZT(NY,NX)))-5.87_r8)
  ELSE
    ENGYB=0.0_r8
  ENDIF
  IF(ENGYD+ENGYB.GT.ZERO)THEN
    HV=1.0E+03*AMAX1(0.0_r8,XVOLT(NY,NX)-VOLWG(NY,NX))/AREA(3,NU(NY,NX),NY,NX)
...
!
    ...
    IF(BKDS(L,NY,NX).GT.ZERO)THEN
      ...
      TORT(M,L,NY,NX)=TortMicporew(THETWT)*(1.0-FHOL(L,NY,NX))
    ELSE
...
!
  IF(BKDS(NUM(NY,NX),NY,NX).GT.ZERO)THEN
    ...
    THETW1=AMAX1(THETY(NUM(NY,NX),NY,NX) &
      ,AMIN1(POROS(NUM(NY,NX),NY,NX) &
      ,safe_adb(VOLW1(NUM(NY,NX),NY,NX),VOLY(NUM(NY,NX),NY,NX))))
    K0=MAX(1,MIN(100,INT(100.0*(AMAX1(0.0_r8,POROS0(NY,NX) &
      -THETWR))/POROS0(NY,NX))+1))
    K1=MAX(1,MIN(100,INT(100.0*(AMAX1(0.0_r8,POROS(NUM(NY,NX),NY,NX) &
    ...
    IF(FLQX.GE.0.0)THEN
      IF(THETWR.GT.THETS(0,NY,NX))THEN
        FLQZ=FLQX+AMIN1((THETWR-THETS(0,NY,NX)) &
          *VOLR(NY,NX),AMAX1(0.0_r8,(THETS(NUM(NY,NX),NY,NX)-THETW1) &
          *VOLY(NUM(NY,NX),NY,NX)))*XNPX
      ELSE
        FLQZ=FLQX
      ENDIF
      FLQR=AMAX1(0.0_r8,AMIN1(FLQZ,VOLW1(0,NY,NX)*XNPX,VOLP1(NUM(NY,NX),NY,NX)))
      FLQ2=AMAX1(0.0_r8,AMIN1(FLQX,VOLW1(0,NY,NX)*XNPX,VOLP1(NUM(NY,NX),NY,NX)))
    ELSE
...
!
  ...
  VOLW1X=AMAX1(0.0_r8,VOLW1(0,NY,NX)+FLWRL(NY,NX))
...
!
  IF(XVOLT(N2,N1).GT.VOLWG(N2,N1))THEN
    ...
    VOLW1X=AMAX1(0.0_r8,VOLW1(0,N2,N1)+WFLXR(N2,N1))
...
!
        IF(NN.EQ.1)THEN
          ...
          IF(ALT1.GT.ALT2)THEN
            QRQ1=AMAX1(0.0_r8,((ALT1-ALT2)*AREA(3,NUM(N2,N1),N2,N1) &
        ...
        IF(NN.EQ.2)THEN
          IF(N4B.GT.0.AND.N5B.GT.0)THEN
            ...
            IF(ALT1.GT.ALTB)THEN
              QRQ1=AMAX1(0.0_r8,((ALT1-ALTB)*AREA(3,NUM(N2,N1),N2,N1) &
          ...
          IF(FLQX.GE.0.0)THEN
            IF(THETW1.GT.THETS(N3,N2,N1))THEN
              FLQZ=FLQX+AMIN1((THETW1-THETS(N3,N2,N1)) &
                *VOLY(N3,N2,N1),AMAX1(0.0_r8,(THETS(N6,N5,N4)-THETWL) &
                *VOLY(N6,N5,N4)))*XNPX
            ELSE
              FLQZ=FLQX
            ENDIF
            FLQL=AMAX1(0.0_r8,AMIN1(FLQZ,VOLW2(N3,N2,N1)*XNPX &
              ,VOLP1(N6,N5,N4)*XNPX))
            FLQ2=AMAX1(0.0_r8,AMIN1(FLQX,VOLW2(N3,N2,N1)*XNPX &
              ,VOLP1(N6,N5,N4)*XNPX))
            !     FLQL1=(THETW1-THETS(N3,N2,N1))*VOLY(N3,N2,N1)
            !     FLQL2=(THETS(N6,N5,N4)-THETWL)*VOLY(N6,N5,N4)
            !     FLQL3=FLQX+AMIN1(FLQL1,AMAX1(0.0_r8,FLQL2))*XNPX
            !     FLQL4=AMAX1(0.0_r8,AMIN1(FLQL3,VOLP1(N6,N5,N4)*XNPX))
          ELSE
          ...
          IF(VOLAH1(N3,N2,N1).GT.ZEROS2(N2,N1) &
            ...
            IF(N.NE.3)THEN
              IF(PSISH1.GT.PSISHL)THEN
                FLWHL(N,N6,N5,N4)=AMAX1(0.0_r8,AMIN1(AMIN1(VOLWH1(N3,N2,N1) &
              ...
              ENDIF
            ELSE
              FLWHL(N,N6,N5,N4)=AMAX1(0.0_r8,AMIN1(AMIN1(VOLWH1(N3,N2,N1)*XNPX &
...
!
        ...
        !
            ...
            IF(FLVC.GE.0.0)THEN
              FLVL=AMAX1(0.0_r8,AMIN1(FLVC,FLVX))
      ...
      !
          ...
          IF(BKDS(N3,N2,N1).GT.ZERO.OR.THETWX(N3,N2,N1)+THETIX(N3,N2,N1).GT.ZERO)THEN
            DTHW1=AMAX1(0.0_r8,THETWX(N3,N2,N1)-TRBW)**3
          ...
          IF(BKDS(N6,N5,N4).GT.ZERO.OR.THETWX(N6,N5,N4) &
            ...
            DTHW2=AMAX1(0.0_r8,THETWX(N6,N5,N4)-TRBW)**3
          ...
          IF(HFLWC.GE.0.0)THEN
            HFLWSX=AMAX1(0.0_r8,AMIN1(HFLWX,HFLWC))
          ELSE
...
!
                IF(IFLGU.EQ.0.AND.test_aneb(RCHGFT,0._r8))THEN
                  ...
                  PSISWT=AMIN1(0.0_r8,-PSISA1(N3,N2,N1)-0.03*PSISO(N3,N2,N1) &
                    ...
                    -0.0098*AMAX1(0.0_r8,DPTH(N3,N2,N1)-DPTHT(N2,N1)))
...
!
                IF(IFLGUH.EQ.0.AND.test_aneb(RCHGFT,0._r8) &
                  ...
                  PSISWTH=-0.03*PSISO(N3,N2,N1) &
                    ...
                    -0.0098*AMAX1(0.0_r8,DPTHH-DPTHT(N2,N1))
...
!
            ...
            !    3,0.0098*AMAX1(0.0_r8,DPTHH-DPTHT(N2,N1))
            !     ENDIF
                ENDIF
!
            ...
            !
                IF(DPTH(N3,N2,N1).GE.DTBLX(N2,N1) &
                  ...
                  PSISWD=XN*0.005*SLOPE(N,N2,N1)*DLYR(N,N3,N2,N1) &
                    *(1.0-DTBLG(N2,N1))
                  PSISUT=AMAX1(0.0_r8,-PSISA1(N3,N2,N1)-0.03*PSISO(N3,N2,N1) &
                    +0.0098*(DPTH(N3,N2,N1)-DTBLX(N2,N1)))
...
!
      IF(VOLWH1(N3,N2,N1).GT.ZEROS2(N2,N1))THEN
        ...
        VOLP1X=AMAX1(0.0_r8,VOLA1(N3,N2,N1)-VOLW1X-VOLI1(N3,N2,N1))
        VOLWH1X=VOLWH1(N3,N2,N1)+TFLWHL(N3,N2,N1)
        VOLPH1X=AMAX1(0.0_r8,VOLAH1(N3,N2,N1)-VOLWH1X-VOLIH1(N3,N2,N1))
        IF(FINHX.GT.0.0)THEN
          FINHL(N3,N2,N1)=AMAX1(0.0_r8,AMIN1(FINHX,VOLWH1X,VOLP1X))
        ELSE
